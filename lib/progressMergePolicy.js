...
import { mergeDailyQuota } from './daily-quota.js'; // Step 3: history 每日配额 union merge
...
export var mergeHistoryData = function (local, server, localNewer) {
  ...
  if (local.dailyLensUsage || server.dailyLensUsage) {
    merged.dailyLensUsage = mergeDailyQuota(local.dailyLensUsage, server.dailyLensUsage);
  }
  if (local.dailySidekickUsage || server.dailySidekickUsage) {
    merged.dailySidekickUsage = mergeDailyQuota(local.dailySidekickUsage, server.dailySidekickUsage);
  }
  return merged;
};
