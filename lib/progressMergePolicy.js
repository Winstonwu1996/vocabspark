...
export var mergeReviewEntry = function (local, server, ctx) {
  ...
  // 快筛"不认识"留痕(screenedUnknownAt)会创建【无任何 SRS 字段】的 stub entry。
  // stub 没有真实活动信号，靠顶层 updatedAt 兜底 recency 可能赢过另一侧的真实复习数据
  // (把 reviewLevel 清成 0、srsUpdatedAt 抹掉)。规则：单侧是 stub 时，非 stub 一方直接赢。
  var isSrsStub = function (e) {
    return !e.srsUpdatedAt &&
      !(Array.isArray(e.reviewHistory) && e.reviewHistory.length) &&
      e.reviewLevel == null && !e.nextReviewDate;
  };
  var localStub = isSrsStub(local);
  var serverStub = isSrsStub(server);

  // recency 更新的赢；完全平局 (同时间戳，极罕见且无信号区分) 保守取较高 level，避免无谓降级
  var winner;
  if (localStub !== serverStub) winner = localStub ? server : local;
  else if (lr > sr) winner = local;
  ...
  return {
    ...
    // 快筛"不认识"留痕：最早一次为准(语义=她第一次自报不认识)；不写进 whitelist 会在任意一次合并后静默丢失
    screenedUnknownAt: earlierISO(local.screenedUnknownAt, server.screenedUnknownAt),
...
export var mergeProgress = function (local, server) {
  ...
  if (local.pet || server.pet) {
    ...
    // 名字不跟喂食数走：按改名时间取更晚的一方（修"改名后第二天变回小 U"）
    merged.pet = resolvePetName(merged.pet, local.pet, server.pet);
  }
...
var PET_SCALAR_FIELDS = ["name", "happiness", "hunger", "equipped", "species", "createdAt", "lastFeedAt", "evolvedAt", "nameUpdatedAt"];

/* 宠物名字独立按【改名时间】归并（用户实测：改名后第二天变回「小 U」）。
 * 根因：name 原本是"搭车"字段——basePet 按 totalFed 谁大谁整包赢，另一台设备
 * 只要多喂几次，它本地的旧名就把改好的名字覆盖掉。
 * 规则：两边都有 nameUpdatedAt → 晚的赢；只有一边有 → 那边赢（显式改名 > 从没改过）；
 * 都没有 → 维持既有 base 行为（旧数据兼容）。 */
var resolvePetName = function (mergedPet, a, b) {
  a = a || {}; b = b || {};
  var ta = toTime(a.nameUpdatedAt);
  var tb = toTime(b.nameUpdatedAt);
  var pick = null;
  if (ta && tb) pick = ta >= tb ? a : b;
  else if (ta) pick = a;
  else if (tb) pick = b;
  if (!pick || !pick.name) return mergedPet;
  var out = Object.assign({}, mergedPet, { name: pick.name });
  if (pick.nameUpdatedAt) out.nameUpdatedAt = pick.nameUpdatedAt;
  return out;
};
...
export var applyProgressGuards = function (incoming, cloud, intent) {
  ...
  if (cp && (cp.totalFed || cp.unlocked || cp.species)) {
    ...
    // 名字不跟喂食数走：按改名时间取更晚的一方（修"改名后第二天变回小 U"）
    mergedPet = resolvePetName(mergedPet, ip, cp);
