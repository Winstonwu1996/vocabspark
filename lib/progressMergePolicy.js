...
/* historyData.profile 也可能是 placeholder (atlas/sample 流自动存的占位, placeholder=true, 非真实填写)。
   同 initialSeed worldview: 一端 placeholder 一端真实 → 真实端整体赢, 不让占位靠更新 updatedAt 覆盖真实
   profile; 两端同态 (都真实/都占位) → 维持 newer 赢 (与原 passthrough 一致)。gated on placeholder, 对
   真实 profile 完全 inert, 零回归。 */
var mergeHistoryProfile = function (a, b, localNewer) {
  if (!a || typeof a !== "object") return b;
  if (!b || typeof b !== "object") return a;
  var aPlace = a.placeholder === true, bPlace = b.placeholder === true;
  if (aPlace && !bPlace) return b;   // a 占位 → 真实 b 赢
  if (bPlace && !aPlace) return a;   // b 占位 → 真实 a 赢
  return localNewer ? a : b;          // 同态 → newer 赢 (同原 passthrough)
};
...
export var mergeHistoryData = function (local, server, localNewer) {
  ...
  if (local.profile || server.profile) {
    merged.profile = mergeHistoryProfile(local.profile, server.profile, localNewer);
  }
  return merged;
};
