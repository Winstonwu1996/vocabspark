 ...
 *     - 配合 ADMIN_UIDS 白名单（见 isAdminUser），创始人/测试号绕过限流。
 */
...
/**
 * 管理员/测试账号白名单——bypass 所有 rate limit。
 * 通过 ADMIN_UIDS env（逗号分隔的 user.id）配置，例：
 *   ADMIN_UIDS=d3906a86-...,a7a84f65-...
 * 用途：
 *   - 创始人/管理员账号不被自家限流卡到（日常开发 + 推广期客服测试）。
 *   - synthetic 监控探针账号（P0-4）走稳定路径。
 * 安全注意：白名单只 bypass rate limit，不绕开认证/守卫。env 未配 → 全部走正常限流。
 */
export function isAdminUser(userId) {
  if (!userId || typeof userId !== "string") return false;
  var raw = process.env.ADMIN_UIDS || "";
  if (!raw) return false;
  var allow = raw.split(",").map(function (s) { return s.trim(); }).filter(Boolean);
  return allow.indexOf(userId) >= 0;
}
