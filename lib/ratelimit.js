...
/**
 * 全局 IP 天花板：**无论有没有 X-User-Id 都生效**，每小时 3000 次。
 *
 * 补的洞：X-User-Id 是未验证的头，每换一个随机值就是一个全新的 300/min 桶，
 * 攻击者每请求换一个 UUID → 任何 per-user 限流都形同虚设（等于开放 LLM 代理，
 * 烧的是我们自己的 DeepSeek/OpenAI key）。加一道恒定生效的 IP 上限即可封顶，
 * 不需要给每个 vocab 请求都加一次 token 验证往返。
 *
 * 阈值取 3000/h 的理由（宁松勿误伤）：
 *   - 重度真实用户约 25 次/5词batch；一个孩子猛学一小时 ~400 次。
 *   - 家庭/学校共享 NAT 3-5 人同时学 ≈ 1200-2000/h，仍有余量。
 *   - 轮换 UID 的脚本原本无上限，现在封到 3000/h —— 量级上的收敛。
 * 触顶属于异常，调用方应 Sentry 告警而不是静默。
 */
export async function checkGlobalIpCeiling(ip) {
  return checkRateLimit(`ipmax:${ip}`, 3000, 60 * 60 * 1000);
}
