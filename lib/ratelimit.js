...
/**
 * 登录用户：每分钟 200 次。
 * 实际单用户峰值 ~30/分（5 词 batch × 3 任务 + 复习），200 充裕；恶意刷脚本仍受限。
 */
export async function checkPerUserLimit(userId) {
  return checkRateLimit(`user:${userId}`, 200, 60 * 1000);
}

/**
 * 游客（未登录）：每小时 100 次。
 *
 * 历史：曾因 chompcloud 家 IP 24h 累计 14700 次怀疑 bot 把上限调到 500/h。
 * 2026-04-29 加诊断 log 后调查：部署 30 分钟内 rl:ip:* 整个命名空间
 * 只有 1 个 key（chompcloud 家 IP），zcard 30，时间间隔分布是典型
 * 人类点击曲线（22/29 < 1s burst）。Vercel logs 期间零 429 触发。
 * 14700 是 30 轮 ralph 迭代 + 手动测试 + Chrome MCP 验证的累积污染，
 * 不是 bot。回到合理值 100/h：
 *   - 单家庭 NAT 5-10 人同时学 vocab 完全够（每人 ~10 次/h）
 *   - 真 bot 打到 100/h 立刻拦截 + 诊断 log 抓到来源
 * 登录用户走 user-level 200/min 不受影响。
 */
export async function checkPerIpLimit(ip) {
  return checkRateLimit(`ip:${ip}`, 100, 60 * 60 * 1000);
}
