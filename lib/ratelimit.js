...
/**
 * 游客（未登录）：每小时 500 次。
 * 100/h 仍不够：实测 chompcloud 家 IP 24h 累计 14700 次（共享 NAT
 * 多人 + 内部测试 + 可能 bot），上限秒爆。500/h 给 NAT 共享 IP
 * 缓冲（5-10 个游客同时学也够）。
 * 真要严格防 bot 应做 cloudflare 级或登录 captcha；推广期临时缓冲。
 * 登录用户走 user-level 200/min 不受影响。
 */
export async function checkPerIpLimit(ip) {
  return checkRateLimit(`ip:${ip}`, 500, 60 * 60 * 1000);
}
