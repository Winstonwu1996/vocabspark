...
import { hasStoryboard, hasLenses, loadStoryboard, listLenses, getLensMeta, preloadStoryboard } from './history-storyboards/index.js';

// storyboard 现在按需加载（见 history-storyboards/index.js 顶部说明）。
// 页面进课前必须先 await 这个，之后所有同步 API 才可用。
export var preloadTopicStoryboard = preloadStoryboard;
...
var nodeToTurn = function(node, idx, allNodes, topicId, lensId) {
  ...
  return {
    ...
    // 一手史料引用：节点上写 sourceRef: '<source id>'，运行时透传，
    // 渲染层据此在这一节下方挂 SourceCard。用独立字段而不是复用 phase='source'——
    // phase 还被 buildStoryboardTurnPrompt 的模板选择用，改它会连带影响 AI 生成。
    _sourceRef: node.sourceRef || null,
...
export var getEffectiveTurns = function(topicId, topicData, lensId) {
  if (hasStoryboard(topicId)) {
    // 这个函数在组件渲染体里被调用 —— 抛错 = 整页白屏（app 级 ErrorBoundary 吃掉）。
    // 三种真实抛法都要接住：① chunk 按需加载失败（网络抖动/部署换版）
    // ② 忘了预加载 ③ 存档里的 lensId 在新版本被改名/删除。
    // 一律退回 topic.conversationTurns，宁可课体验降级也不能白屏。
    try {
      var sb = loadStoryboard(topicId, lensId);
      return sb.map(function(node, idx) { return nodeToTurn(node, idx, sb, topicId, lensId); });
    } catch (e) {
      if (typeof console !== 'undefined') {
        console.error('[history-runtime] getEffectiveTurns 降级:', topicId, lensId, e && e.message);
      }
      // lensId 无效但课本身在 → 再试一次默认 lens，比直接掉到 v1 兜底更接近原意
      if (lensId) {
        try {
          var sb2 = loadStoryboard(topicId, null);
          return sb2.map(function(node, idx) { return nodeToTurn(node, idx, sb2, topicId, null); });
        } catch (e2) { /* 默认 lens 也不行 → 落到下面 */ }
      }
    }
  }
