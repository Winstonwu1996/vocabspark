...
var nodeToTurn = function(node, idx, allNodes) {
  ...
  return {
    ...
    role: 'ai',
    move: node.phase,
    expectsInput: !!node.expectsRealAnswer || isRetrievalGate,
    autoAdvance: false,
    inputPrompt: isRetrievalGate ? RETRIEVAL_PROMPT : node.engagementHook,
...
export var getEffectiveTurns = function(topicId, topicData, lensId) {
  if (hasStoryboard(topicId)) {
    ...
    return sb.map(function(node, idx) { return nodeToTurn(node, idx, sb); });
  }
