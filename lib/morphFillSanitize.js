 ...
 */
/* 选项重复检查（所有 4 选项题型通用）。
 * 线上实测 bug（hummock）：A/C/D 三个选项都是 "hummock"，学生选 C 与正解 A 是同一个词
 * 却被判错 —— "选对了还判错"是最伤信任的硬伤，必须程序拦（模型偶尔就是会吐重复）。
 * 大小写/空格不敏感；不足 2 个有效选项也算坏题。
 * 返回 { bad, reason }。
 */
export var checkDuplicateOptions = function (options) {
  if (!options || typeof options !== "object") return { bad: true, reason: "options_missing" };
  var seen = {};
  var count = 0;
  var keys = Object.keys(options);
  for (var i = 0; i < keys.length; i++) {
    var raw = options[keys[i]];
    if (typeof raw !== "string") continue;
    var v = raw.trim().toLowerCase();
    if (!v) continue;
    count++;
    if (seen[v]) return { bad: true, reason: "duplicate_option:" + v };
    seen[v] = true;
  }
  if (count < 2) return { bad: true, reason: "too_few_options" };
