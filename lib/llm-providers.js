...
export const buildProviders = () => {
  ...
  // OpenAI 兜底：DeepSeek(跨境)失败时的独立退路。美国节点、OpenAI 原生兼容(参数/响应同格式)、
  // 中文够用。排在 Gemini 前(Gemini 免费额度限流严)。key 未配则跳过(此块不激活)。
...
async function callProvider(provider, system, message, maxTokens, timeoutMs, opts = {}) {
  ...
  // V4-flash 的思考模式默认可能是开的（旧 deepseek-chat 是关的）。显式关闭，
  // 否则响应会被推理串污染 → teach/guess 的 JSON 解析全崩 + output token 暴涨。
  if (provider.family === "deepseek") body.thinking = { type: "disabled" };
