...
export var buildHistorySystemPrompt = function(opts) {
  ...
  // 5-5 R2: placeholder profile (用户选了"稍后再说") → AI 用通用方式,不强行带入个人化字段
...
export var buildTurnPrompt = function(turn, opts) {
  ...
  // 5-5: lens 模式下节点数随 storyboard 变化(12 节 prewritten);老 v1 模式仍 13 轮
  var totalTurns = opts.totalTurns || 13;

  var blocks = [
    "# 本轮（第 " + turn.n + " / " + totalTurns + " 轮,move=" + turn.move + "）",
    "",
...
export var buildDefinitionEvalPrompt = function(concept, userAnswer) {
  // schema 双兼容：老 topic 用 concept.defEn/defCn，新 lens-based topic 用 concept.definition.en/cn
  // 旧字段缺失会让评估器拿到 undefined 标准定义 → 评分全失效（Codex 2026-05-25 抓出）
  var defEn = concept.defEn || (concept.definition && concept.definition.en) || "";
  var defCn = concept.defCn || (concept.definition && concept.definition.cn) || "";
  return [
    "# 评估任务",
...
export var buildWorldviewSnapshotPrompt = function(topicId, transcript) {
  return [
    "# 任务",
    "你刚陪一个中国家庭背景的中学生上完一个历史 Topic（topicId=" + topicId + "）。",
...
export var buildFreeChatSystemPrompt = function(opts) {
  ...
  var profile = opts.profile || "";
  var userName = opts.userName || "你";
  var topicTitle = topic ? topic.title.cn : "本课主题";

  // 主题边界按 topic 数据生成 —— 曾写死 Magna Carta 的人物/事件/概念白名单，
  // 但本 builder 服务全部 71 门课：古埃及课里判离题却拿大宪章当标准，且拉回话术
  // 会冒出「你对 King John 哪部分最好奇」。concepts/oneLineHook 71/71 全覆盖，
  // perspectives 仅 1 门有 → 有则用，无则省略该行。
  var conceptList = ((topic && topic.mustMemorize && topic.mustMemorize.concepts) || [])
    .map(function(c) { return c.cn ? c.en + "（" + c.cn + "）" : c.en; })
    .filter(Boolean);
  var peopleList = ((topic && topic.perspectives) || [])
    .map(function(p) { return p.label && (p.label.cn || p.label.en); })
    .filter(Boolean);
  // oneLineHook 长短差异极大（唐宋那条 300+ 字），截断防撑爆 prompt
  var topicGist = (topic && topic.oneLineHook && topic.oneLineHook.cn) || "";
  if (topicGist.length > 120) {
    var cut = topicGist.slice(0, 120);
    // 在最后一个自然停顿处收尾；找不到就硬截（绝不能把整句吃空）
    var stop = Math.max(cut.lastIndexOf("→"), cut.lastIndexOf("。"), cut.lastIndexOf("，"), cut.lastIndexOf(" "));
    topicGist = (stop > 60 ? cut.slice(0, stop) : cut).trim() + "…";
  }
  // 拉回话术里的锚点用短名（中文优先），别把 "the X Question（中文）" 整串塞进话术
  var firstConcept = ((topic && topic.mustMemorize && topic.mustMemorize.concepts) || [])[0];
  var anchorTerm = (firstConcept && (firstConcept.cn || firstConcept.en)) || topicTitle;

  var scopeLines = ["今天聊的范围 = " + topicTitle + " 及其历史延伸："];
  if (topicGist) scopeLines.push("- 本课主线：" + topicGist);
  if (peopleList.length) scopeLines.push("- 人物：" + peopleList.join(" / "));
  if (conceptList.length) scopeLines.push("- 概念：" + conceptList.slice(0, 8).join(" / "));
  scopeLines.push("- 也算范围内：这些人物/概念的历史来龙去脉、同期其他文明的对照、以及它们和今天的关联");
  ...
  return [
    ...
    "- 提到当代政治 / 校规 / 法庭新闻 / 别的历史时期",
    "- 做法：先接住她的问题，再用一句话把它接回「" + anchorTerm + "」这类本课概念上",
    "",
    "**Level C 离题** — 温和拉回：",
    ...
    "",
    "## 拉回话术（按次数升级）",
    "第 1 次离题：「这个我也想聊，但今天主题是 " + topicTitle + "。下次有相关的 Topic 我陪你聊？现在你对「" + anchorTerm + "」哪部分最好奇？」",
    "第 2 次离题：「还是 " + topicTitle + " 相关的故事吧 — 你想从哪个角色讲起？」",
    "第 3 次离题：直接温和收尾「今天先到这吧，建议你睡前把今天聊的东西在脑子里过一遍」",
    "",
    "# 史实可信度（硬规则 · Gate3 §8）",
    "她会问课件之外的延伸问题（例：「Stanford 是不是靠华工血汗建的？」）。答之前先判断你的把握度：",
    "- **有把握、是公认事实**：正常答。",
    "- **课件外、你不完全确定**：先说一句「这超出了我们今天聊的范围，我给你一个谨慎的说法」，再答——不要把推测讲成定论。",
    "- **有争议 / 你没把握**：明说「这个历史学家也有不同说法」或「我不太确定」，给不同角度——**绝不编造确定结论、具体数字或引文**。",
    "- 两个方向都禁止：不为了「听起来肯定」把不确定说成定论；也不为了「让她舒服」软化或回避史实。",
    "",
    "# 输出约束",
    "1. **50-200 中英混合字符**（标点也算）",
    "2. **朋友语气，禁止下结论**（不要说「答案是...」「记住：...」）",
    "3. **中国概念用中文**（不要英译唐律/天子，那只会困惑她）",
    "4. **不重复 Topic 主线已经讲过的内容**（她已经走完整条主线）",
    "5. **可以反问**（这是好习惯：「你觉得呢？」「这个像不像你之前说的 X？」）",
    "",
    "# 用户画像",
    profile || "（暂无 — 用通用中国家庭背景 ESL 中学生默认，不要假设具体年级）",
  ]).join("\n");
};
...
export var buildNarrativeSystemPrompt = function(opts) {
  ...
  blocks.push('');
  blocks.push('## ⚠️ 用户问到「材料之外」时怎么办（史实可信度硬规则 · Gate3 §8）');
  blocks.push('她会自由追问，问题可能超出上面的 narrative。回答前先判断这个问题的「材料归属」，再决定怎么答：');
  blocks.push('- **课件内**（上面 narrative 里直接有）：放心答，事实准确。');
  blocks.push('- **课件外但相关**（narrative 没有，但与本 Topic 相关、你也有把握）：先说一句「这超出了我们这节的材料，我给你一个谨慎的说法」，再答——不要把推测讲成定论。');
  blocks.push('- **不确定 / 有争议**（史学界有分歧，或你没把握）：**明确说出来**——「这个历史学家也有不同说法」或「我不太确定」——给已知的不同角度，**绝不编造确定结论、具体数字或引文**。');
  blocks.push('- 两个方向都禁止：不要为了「听起来肯定」把不确定说成定论；也不要为了「让她舒服」软化或回避史实。');
  blocks.push('');
  ...
  // ── 用户背景 ──
  blocks.push('# 学生背景（' + userName + '）');
  blocks.push('- ' + userAge + ' 岁，在' + userSchool + '上学；中国家庭背景，**中国史是 TA 的 home base**（唐宋 / 天命 / 科举这类 TA 通常比美国同学熟）');
  blocks.push('- 英文还在 ESL 阶段（Lexile 大致 800-1000）');
  blocks.push('- 整体偏好：喜欢被当成对话对象一起聊、不喜欢被居高临下讲课');
  blocks.push('（具体年级 / 性格 / 强项以下面「她的具体画像」为准——那是这个用户真实累积的，不要套用上面的笼统默认）');
...
export var buildStoryboardTurnPrompt = function(node, opts) {
  ...
  blocks.push('**⚠️ 严格遵守上限**——不要超过节点指定字数——画面感 > 全面覆盖——宁可少一件事讲透不要多件事都浅尝');
  blocks.push('');

  // ── ⭐ Anti-hallucination 硬约束 ──
  blocks.push('## ⚠️ 事实约束（防止 hallucination）');
  blocks.push('- **所有具体日期 / 人名 / 地点 / 数字必须从 narrative kernel（system prompt 上方提供的 narrative body）取**');
  blocks.push('- **绝对不要发明** narrative 里没有的事实——不要凭"听起来合理"猜年份');
  blocks.push('- 如果 deliverGoal 提到一个日期但 narrative 里没明示——**优先 narrative**——不要硬塞 deliverGoal 的日期');
  blocks.push('- 如果你不确定某个事实——**省略**比**编造**好');
  blocks.push('- 例：King John 1166 年生（narrative 给的事实）——**不要写"1466 年"**——这种数字错误绝对不可接受');
  blocks.push('');

  // ── 跨节点重复检测 ──
  blocks.push('## ⚠️ 不要重复前面节点已讲过的内容');
  blocks.push('- 你之前几轮已经讲过的具体场景 / 人物背景——**不要重新介绍**');
  blocks.push('- 如果 deliverGoal 明示"前面已讲过 X——这里只讲 Y"——**严格遵守**——直接进 Y 不要回头复述 X');
  blocks.push('- 故事是**前进**的——每一轮**推进**到新场景——不是**重温**');
  blocks.push('');
...
//   2. 答（100-200 字）——先分级（课件内 / 课件外但相关 / 不确定·争议），课件外明确声明「超出本节材料，谨慎解释」，争议明说分歧，绝不编造确定结论（Gate3 §8 runtime 前置）
//   3. 夸用户（"这个角度想得很好" 类）
...
export var buildBranchPrompt = function(opts) {
  ...
  blocks.push('  先判断这个问题的「材料归属」，再决定怎么答（硬要求——保护史实可信度，防止过度肯定或过度软化）：');
  blocks.push('  - **课件内**（narrative 里直接有答案）：放心答——事实准确 + 明朝密度。');
  blocks.push('  - **课件外但相关**（narrative 没有，但与本主题相关、你也有把握）：先明确说一句「这超出了我们这节的材料，我给你一个谨慎的说法」，再答——不要把推测讲成定论。');
  blocks.push('  - **不确定 / 有争议**（史学界有分歧，或你没把握）：**明确说出来**——「这个历史学家也有不同说法」或「我不太确定」——给已知的不同角度，**绝不编造确定结论、具体数字或引文**。');
  blocks.push('  - 任何情况都**绝不矛盾**于 narrative；保持明朝密度——具体人/时间/场景——不是抽象解释。');
  blocks.push('  - ❌ 不要说"这个跟我们今天的故事无关"——任何问题都值得答（课件外的也答，只是要加上面的谨慎声明）。');
  blocks.push('');
