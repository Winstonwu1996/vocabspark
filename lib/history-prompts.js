...
export var deriveTurnObjective = function(turn) {
  ...
  // 默认 mapping
  var map = {
    'hook':                 { o: '从直觉切入：让她对核心矛盾产生第一感觉', np: '第 1 节"一句话锚点" + 第 2 节舞台' },
    ...
    'geo':                  { o: '把她带入舞台地理 — 让她意识到地理塑造冲突', np: '第 2 节"地理 L0"' },
    ...
    'roleplay':             { o: '让她代入第一人称角色，从内部感受', np: '第 3 节"主角们"' },
...
// ── 构造 narrative-driven 单轮 turn prompt（短，目标单一）──
export var buildNarrativeTurnPrompt = function(turn, opts) {
  ...
  blocks.push('## 这一轮的唯一目标（你只做这一件事）');
  blocks.push(turn.objective || '(未指定)');
  blocks.push('');

  if (turn.narrativePoint) {
    blocks.push('## 故事中的位置（在 narrative 里的哪一段）');
  ...
  if (lastUserAnswer && lastUserAnswer.trim()) {
    blocks.push('## 她刚才说');
    blocks.push('「' + lastUserAnswer.trim() + '」');
    blocks.push('');
    blocks.push('## 评估再过渡');
    blocks.push('先用 1 句话真诚回应她说的（共情或追问，不是套话），再 1 句过渡到本轮目标。');
    blocks.push('**禁止**：复述她原话 / 忽略她直接进下一轮 / 套话（"很好"/"有意思"）');
    blocks.push('');
  } else {
    blocks.push('## 这是开场轮（她还没说话）');
    blocks.push('直接进入本轮目标，不需要先评估什么。');
    blocks.push('');
  }
  ...
  if (turn.mustIntroduce && turn.mustIntroduce.length > 0) {
    ...
    blocks.push('自然嵌入对话，不要堆清单。');
    blocks.push('');
  }

  // ── 输出约束（简洁 + 末尾 reaffirm 关键规则）──
  blocks.push('## 输出约束（写之前再 check 一遍）');
  blocks.push('- 长度 ≤ ' + (c.maxChars || 120) + ' 字（中英混合计数）');
  ...
  if (c.allowedActions && c.allowedActions.length > 0) {
    blocks.push('- 允许的动作（最多 1-2 种）：' + c.allowedActions.join(' / '));
  }
  blocks.push('- **单一动作绝对纪律**：你这一轮的回应只能做一件事');
  blocks.push('');

  blocks.push('写出你这一轮的回应（直接输出，不要包 markdown 块，不要解释你的思路）。');
