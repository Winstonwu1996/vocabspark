...
export var buildNarrativeSystemPrompt = function(opts) {
  ...
  blocks.push('Narrative 里的具体细节（场景、引语、数字、对照、人物动机）你可以自由取用。');
  blocks.push('');
  blocks.push('═══════════════════════════════════════');
...
export var deriveTurnObjective = function(turn) {
  ...
  var map = {
    ...
    'setup-A':                    { o: '引入文明 A — 厚 deliver（人物/制度/动机），让她有原料思考', np: '第 2 节"制度 L3" + 第 3 节"主角"A 方' },
    'setup-B':                    { o: '引入文明 B + Pin 时刻（"我们刚学了 A 现在加 B"）', np: '第 2 节"制度 L3" + 第 3 节"主角"B 方' },
    'setup-C':                    { o: '引入文明 C，三方齐全（再次 Pin）', np: '第 2 节"制度 L3" + 第 3 节"主角"C 方' },
    'setup-latin':                { o: '引入拉丁基督教（教皇 Urban II + Clermont + plenary indulgence + 教权 vs 王权张力）— 厚 deliver', np: '第 2 节"制度 L3 ① 拉丁" + 第 3 节"主角"拉丁方' },
    'setup-islam':                { o: '引入伊斯兰世界（Caliph + Sultan + 逊尼/什叶分裂）+ Pin 时刻', np: '第 2 节"制度 L3 ③ 伊斯兰" + 第 3 节"主角"伊斯兰方' },
    'setup-byzantium-pivot':      { o: '引入拜占庭（Alexios I 求援 + Anna Komnene） + 1204 反攻自家震撼时刻 — 第三阵营 + 关键转折合并', np: '第 2 节"制度 L3 ② 拜占庭" + 第 3 节"主角"拜占庭方 + 第 4 节 1204' },
    'pin':                        { o: '显式让她 recap 之前学的——本轮唯一任务是巩固，不引新内容', np: '前面所有已介绍内容' },
    // primary source
    ...
    'seal':                       { o: '收束今天的对话 — 留一个开放问题不下结论', np: '第 11 节"思考问题"' },
  };
...
export var deriveTurnConstraints = function(turn) {
  ...
  var isSeal = turn.move === 'seal';
  var isGeo = turn.move === 'geo';
...
// ── 构造 narrative-driven 单轮 turn prompt（4-slot Deliver-Provoke 节奏 + Anti-Guessing Test）──
//
// 教学心法：每轮"微讲课 + 强迫应用"两段式
//   Slot 1 Empathy   (10-20 字，仅当 user 说话后)  - 接住她原话具体点
//   Slot 2 Bridge    (10-20 字，可选)              - 从她的话桥到新内容
//   Slot 3 Deliver   (30-60 字)                    - 交付 narrative 里具体信息（厚到能思考）
//   Slot 4 Provoke   (15-30 字)                    - 单一问题，要求她 USE/EVALUATE/COMPARE 刚交付的信息
//
// Anti-Guessing Test：写完后 self-check —
//   "给定刚 deliver 的信息，13 岁能用系统推理答这个问题吗？还是只能瞎猜？"
//   只能瞎猜 → 加厚 deliver 或换问题
export var buildNarrativeTurnPrompt = function(turn, opts) {
  ...
  // ── 4-slot Deliver-Provoke 节奏（核心写作公式）──
  blocks.push('## 写作公式：4-slot Deliver-Provoke');
  blocks.push('严格按这个顺序写，每段不要超长：');
  blocks.push('');
  if (hasUserAnswer) {
    ...
    blocks.push('从她说的桥到本轮新内容（如果自然就连，不自然就直接 Slot 3）');
    blocks.push('');
  } else {
    blocks.push('（开场轮，跳过 Empathy + Bridge，直接 Slot 3）');
    blocks.push('');
  }
  blocks.push('**Slot 3 — Deliver（30-60 字）⭐ 关键 slot**');
  blocks.push('交付 narrative 里的**具体信息**——人物 / 时间 / 数字 / 场景 / 引语 / 制度细节。');
  blocks.push('**这一段必须厚到她能用系统推理思考**，不是泛泛描述。');
  blocks.push('禁止：发明 narrative 里没有的细节 / 抽象概念堆砌 / 浮于表面');
  blocks.push('');
  blocks.push('**Slot 4 — Provoke（15-30 字）⭐ 单一问题**');
  blocks.push('问一个**让她 USE / EVALUATE / COMPARE 刚 Deliver 信息**的问题。');
  blocks.push('禁止：多个问题 / 笼统"你觉得呢？" / 跟 Slot 3 信息脱节的问题');
  blocks.push('');

  // ── Anti-Guessing Test ──
  blocks.push('## ⚠️ Anti-Guessing Test（写完后自检）');
  blocks.push('问自己：**给定我刚 Deliver 的信息，13 岁能用系统推理答这个问题吗？还是只能瞎猜？**');
  blocks.push('- ❌ 只能瞎猜（信息不够 / 问题脱节）→ 加厚 Slot 3 或换 Slot 4 问题');
  blocks.push('- ✅ 能用刚学的信息推理出来 → 通过');
  blocks.push('');
  ...
  // ── 输出约束 ──
  blocks.push('## 输出约束');
  blocks.push('- 总长度 ≤ ' + (c.maxChars || 120) + ' 字');
  ...
  blocks.push('- **单一动作绝对纪律**：4 个 slot 是**节奏分段**不是 4 个动作；整体是"一段微讲课 + 一个问题"');
  blocks.push('');
