...
// 注 (Codex round4 P2-g): 不提供 refund —— eventIds 走 union 合并 (append-only), 一旦同步/其它
// tab 有副本, 移除会被下次 merge 复活。union 语义下「记了即永久」, 故 reserve 后即便 API 失败也不退,
// 调用方接受「失败消耗 1 次」(罕见, 相对每日额成本可忽略), 换全同步状态下的正确强制。
