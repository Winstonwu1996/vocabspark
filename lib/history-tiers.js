...
export function computeGateAccess(tierInfo, topicId, lensId, learningReceipts) {
  ...
  if (state === 'error') {
    // Codex P2-c (round4): grandfather 是**永久权利**, 必须先于 fail-closed 判定 —
    // 否则 /api/stripe/check-subscription 一次瞬时失败就把已学完 Basic/Pro lens 的用户
    // 锁在自己学过的内容外。已学过的 lens → view-only-grandfathered, 不受 tier 查询失败影响。
    if (lensCompletedFrom(learningReceipts, topicId, lensId)) return 'view-only-grandfathered';
    // 其余 fail-closed: 付费课 (basic/pro) 不放行; 免费课 (guest/free 白名单) 仍放行
