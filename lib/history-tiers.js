...
/**
 * lens 级 access 决策. v1.2 §10 G + Step 4b-3 修正:
 *   grandfather 是「缺 tier 但已学过」的**回退**, 不是无条件优先。
 *   - tier 够 → 'allow' (含已完成者重学 —— 付费用户重访已学 lens 直接进, 不该被劝升级)
 *   - tier 不够但已学过 → 'view-only-grandfathered' (永久只读可见 + 升级才能重学)
 *   - tier 不够且没学过 → 'deny'
 *
 * @param {string} topicId
 * @param {string} lensId
 ...
 * @returns {string} 'allow' | 'view-only-grandfathered' | 'deny'
 *
 * 用法:
 *   - 'allow': 正常进入对话
 *   - 'view-only-grandfathered': 只读占位屏 (4b-3 A) + 「重新学需要 Basic/Pro」按钮
...
export function canAccessLens(topicId, lensId, userTier, hasCompletedLens) {
  if (canAccessTopic(topicId, userTier)) return 'allow';
...
export function computeGateAccess(tierInfo, topicId, lensId, learningReceipts) {
  ...
  if (state === 'error') {
    // tier 查询失败 (check-subscription 耗尽重试):
    // - 免费/guest 课 → allow (fail open, 免费内容不因网络抖动被挡)。
    // - 付费课 → error-blocked (网络重试 modal, 诚实: 无法验证 tier, 刷新即解析真值)。
    // 注 (4b-3 / Codex round1): 不再对已学付费 lens 返回 view-only-grandfathered —— view-only 现在是
    // 「升级重学」占位屏, tier 未知时臆测用户「缺 tier」会误劝可能仍在付费的用户; 且完成的免费课
    // 已由下面的 free/guest→allow 接住。grandfather 只在**已知**缺 tier (free/guest state) 时才该出现。
