...
export function useSimplifiedMode(opts) {
  ...
  // SSR/水合一致性：首屏（server + client 第一次 render）都不读 window，保证 server HTML
  // 与 client 首个 render 完全一致；否则 embedded URL 会触发 "Text content did not match"
  // hydration error。mount 后才打开同步 URL fallback。
  ...
  return useMemo(function () {
    ...
    // 仅水合后启用，避免 server/client 首屏分裂
    var urlFlags = (hydrated && typeof window !== 'undefined') ? new URLSearchParams(window.location.search) : null;
  ...
  }, [pendingRole, embedded, fromAtlas, hasLensesForTopic, hydrated]);
}
