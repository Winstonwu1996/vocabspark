...
var KEEP = 2;
// Codex P2: blob 过大时不备份 —— 否则 premigration + 滚动多份全量副本会挤占同一 localStorage 配额,
// 把主 blob 逼到 QuotaExceededError, 后续 doSave 存不进去 (备份反而害了主存档)。
// 字符数上限 (localStorage 多按 UTF-16 计, 1 char≈2 bytes); 500K 字符 ≈ 1MB, 留足主 blob + 副本空间。
// 超限的大用户跳过备份 (罕见; 优先保证主存档不被挤爆)。
var MAX_BACKUP_CHARS = 500000;

function _ls() {
  // Codex P2: 隐私模式/受限环境下连「读 window.localStorage」本身都可能抛 SecurityError →
  // 包 try/catch, 不可用时返回 null, 保证 backupOnBoot 永不在 mount 时抛出阻塞页面。
  try {
...
export function snapshotPreMigrationOnce() {
  ...
  try {
    ...
    if (cur != null && cur.length <= MAX_BACKUP_CHARS) LS.setItem(PREMIGRATION_KEY, cur);
  } catch (e) {}
}
...
export function snapshotBackup() {
  ...
  try {
    ...
    if (cur.length > MAX_BACKUP_CHARS) return; // Codex P2: blob 过大 → 跳过, 不挤占主存档配额
    ...
    // Codex P2: 先修剪到 KEEP-1 份「再」写新备份 —— 否则 quota 将满时先写第 4 份会抛, catch 静默退出,
    // 永远轮转不动 (旧备份没删, 新的也没写)。先删最旧腾地, 给新备份留空间。
    while (baks.length > KEEP - 1) {
...
// Codex P1: 登出 / 重置时必须连备份一起清 —— 否则共享浏览器上备份键仍留着完整进度/画像 blob,
// 可被恢复或在另一个账号下推上云 (隐私泄露)。在所有清本地数据处一并调本函数。
export function clearBackups() {
  var LS = _ls();
  if (!LS) return;
  try {
    LS.removeItem(PREMIGRATION_KEY);
    var baks = listBackups();
    baks.forEach(function (b) { try { LS.removeItem(b.key); } catch (e) {} });
  } catch (e) {}
}
