...
export var METHOD_SCHEMAS = {
  ...
  mnemonic: '🧠 谐音联想 mnemonic\n{"type":"mnemonic", "soundAlike":"中文谐音(≤12字, 只给成品)", "interpretation":"→ 含义解释", "anchor":"（可选）画像场景"}\n  ❗soundAlike 必须【音近 + 意思指向词义】双满足。只音像、意思无关 = 废线索（反例：hybrid 含义"混合的"配"还不赖的"——毫无关系，学生白背一句）。\n  ❗只输出最终谐音，绝不写思考过程/比较/自我怀疑（反例：「读着像X? 不如Y——但更贴近Z」= 认知负担），也不写"近似/谐音/音近"这类自我开脱。编不出自然又意通的就留空 ""，改用画面/词根方法。',
...
var VISUAL_ANCHOR_RULES = '\n【锚点质量铁律】\n' +
  ...
  '- 医疗/身体/疾病词：基调必须尊重生命、指向治愈(🏥💗 而非 🔪🩸)。严禁器官↔食物的比喻或切割猎奇画面（实测反例：hysterectomy 配 🍓🔪「像切草莓一样切掉子宫」，学生反馈很恶心）\n' +
...
export var buildQuizAuditPrompt = function(kind, itemJson) {
  ...
  var byKind = {
    ...
    spectrum: "3a. 若题目含 soundHint（谐音线索）：谐音的中文必须【既音近、又指向词义】。只音像但意思与词义无关（真实案例：hybrid'混合的'配'还不赖的'）= 废线索，fail 并说明应留空改用画面线索。\n" +
             "3. 这是学后即时巩固题。若题面在问『哪个是【某个特定义项】』（多义词义项辨别，如 run 的『经营』义）：干扰项按设计【就是】目标词的其他真实义项，此时只审干扰项是否确属【不同义项】且与目标义项无重叠——不得因『干扰项也是该词合法用法』打回。其他题型（含『哪个场景最契合核心含义』）：干扰项必须是把正解义项代入后明显不通的场景/用法。❗只要有干扰项在初中生标准下【也是该词同一义项的正确用法】，即 fail——严禁靠『更侧重/更典型/更精准』这类自造精细区分定唯一解（真实案例：hereditary 把'蓝眼睛遗传自祖父母'判错、选'城堡世袭诅咒'为唯一正解）。\n",
...
export var buildClassifyPrompt = function(word, profile, learned) {
  ...
  return "单词：" + word + "\n" +
    ...
    "【方法池 — type 字段必须是以下 9 个英文小写 key 之一（不要带 emoji、不要带中文）】\n" +
    "root | image | nuance | scale | etymology | collocation | antonym | family | microstory\n" +
    "❗mnemonic（中文谐音）已停用：实测生成的谐音多为无意义凑音（如 hypnotism 配「嗨皮 no 提死我」），\n" +
    "  是纯认知负担。难记词一律改用 image（画面）/ etymology（词源）/ microstory（微故事）。绝不输出 mnemonic。\n\n" +
    "各方法简介：\n" +
    ...
    "C → root + family\n" +
    "D → image + microstory（难记词靠画面/故事锚定，不用谐音）\n" +
