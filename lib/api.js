...
export var buildQuizAuditPrompt = function(kind, itemJson) {
  ...
  var byKind = {
    guess: "3. 这是学前 priming 猜词题：语境句必须含有指向正解的线索（若≥2 个选项都能自然填入空格，就是线索不足的纯抽奖题，fail）。4 个选项必须全是初中生一眼认识的高频词，出现生僻词即 fail。\n",
    spectrum: "3. 这是学后即时巩固题。若题面在问『哪个是【某个特定义项】』（多义词义项辨别，如 run 的『经营』义）：干扰项按设计【就是】目标词的其他真实义项，此时只审干扰项是否确属【不同义项】且与目标义项无重叠——不得因『干扰项也是该词合法用法』打回。其他题型：干扰项必须是把正解义项代入后明显不通的场景/用法，不能是同一义项的合法用法排座次。\n",
