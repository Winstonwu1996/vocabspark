...
import { mergeStates, validateMerged, pruneCompletedInProgress } from './syncMerge.js';
...
export function createSyncClient(options) {
  ...
  async function _pushSnapshot(dataToPush, intent) {
    ...
    if (r.status === 409) {
      ...
      if (conflict.serverData) {
        ...
        // 已完成课的过期续点不随 409 union 复活 (Codex P2): union 会把服务端残留的 inProgress 并回来。
        // 对 vocab-only blob inert (无 historyData.inProgress/completedTopics), 对 history 是正确不变量。
        pruneCompletedInProgress(merged);
        ...
        if (r2.ok) {
          ...
          if (conflict2.serverData) {
            ...
            pruneCompletedInProgress(merged2); // 同上: 二次 409 union 也不复活过期续点
  ...
  // 「记住进度」主动同步专用: 先拉云合并再解锁推送 (Codex P1)。
  // 直接 setCloudReady(true)+syncToCloud 会在「从未拉过云」时 (如先开 /history, user_progress 仍是迁移
  // 默认 version 0) 用本地旧 blob 走写路径覆盖云端 history 字段 (inProgress/bridgeQueue/不相交的
  // completedTopics)。本方法复用登录时的安全序: 拉云 → 字段级 union 合并 → 落本地 → 解锁 → 推 merged。
  // 拉云失败 (网络/HTTP) → 不解锁、不推、报 error, 返回 false 让调用方提示重试 (绝不覆盖云端)。
  async function publicEnablePushFromCloud() {
    // 与既有 in-flight sync 串行 (Codex P2): 单例假设同步操作串行, 不在别的 sync 跑着时插入拉云重 pin
    // 回调 (会让那次 sync 的 409/状态走错页回调、争抢共享 version/快照)。先等当前 in-flight 结束, 再占锁。
    var waited = 0;
    while (_syncInFlightRef.current && waited < 6000) {
      await new Promise(function (r) { setTimeout(r, 150); });
      waited += 150;
    }
    if (_syncInFlightRef.current) { // 仍在忙 → 不强插, 让用户稍后重试
      onSyncStatus('error', { version: _syncVersionRef.current, lastSyncAt: _lastSyncAtRef.current });
      return false;
    }
    // 占串行锁: 此后到 finally 之间, 并行 _doSync 会在自身入口看到 inFlight=true → 标 pending 退避, 不并发。
    // (从上面的 while 退出到这一行无 await, 单线程原子, 中间不会被 _doSync 抢跑。)
    _syncInFlightRef.current = true;
    _pinCallbacks();
    var ok = false;
    try {
      var u = getUser();
      if (!u) return false;
      onSyncStatus('syncing', { version: _syncVersionRef.current, lastSyncAt: _lastSyncAtRef.current });
      var cloudRes = await loadFromCloudInternal(u.id); // 成功后 _syncVersionRef = 服务器版本
      if (!cloudRes.ok) {
        onSyncStatus('error', { version: _syncVersionRef.current, lastSyncAt: _lastSyncAtRef.current });
        return false;
      }
      // 拉云期间登出/换号 → 放弃 (Codex P2): 不把上个账号的云端数据写进共享 blob、更不解锁推到新账号 token 下。
      var uAfterLoad = getUser();
      if (!uAfterLoad || uAfterLoad.id !== u.id) return false;
      if (cloudRes.data) {
        var localData = await loadLocalSnapshot();
        var uAfterLocal = getUser(); // loadLocalSnapshot 也是 await, 再确认一次
        if (!uAfterLocal || uAfterLocal.id !== u.id) return false;
        var merged;
        try {
          merged = mergeStates(localData || {}, cloudRes.data);
          if (!validateMerged(merged, cloudRes.data)) merged = cloudRes.data;
        ...
        // 已完成课程的过期续点不随 union 复活 (Codex P2): 丢弃早于完成时间的 inProgress, 重学的更新续点保留。
        pruneCompletedInProgress(merged);
        await saveLocalSnapshot(merged);
        onCloudData(merged);
      }
      _cloudReadyRef.current = true; // 已拉过云 → 解锁 push 安全 (后续学完课自动同步)
      ok = true;
      return true;
    } finally {
      _syncInFlightRef.current = false; // 释放串行锁
      // 仅成功路径才推送 (账号变更/失败的中途 return 不推, 不写脏数据)。锁已释放, 走正常 _doSync;
      // 顺带刷出 in-flight 期间被退避排队的 pending。
      // 不变量 (workflow review): saveLocalSnapshot 必须同步落盘 —— 上面 await 它在同一 tick 完成, 中途无法
      // 插入切页/换号; 若将来改异步持久化 (IndexedDB/SW), 这里的 publicSyncToCloud 与账号校验需重新评估。
      if (ok) publicSyncToCloud();
      else if (_syncPendingRef.current) { _syncPendingRef.current = false; publicSyncToCloud(); }
    }
  }
  ...
  return {
    ...
    /**
     * 主动同步: 先拉云合并再解锁推送, 安全建立首次 push (避免未拉云覆盖 version-0 云端)。
     * 返回 Promise<boolean>: true=已拉云并发起推送; false=拉云失败 (未推、未解锁)。
     */
    enablePushFromCloud: publicEnablePushFromCloud,
