...
/**
 * date 合法性: 必须是 'YYYY-MM-DD' 字符串 (P1-1 Codex Step 3 审计)。
 */
export function isValidQuotaDate(d) {
  return typeof d === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(d);
}

/**
 * 合并核心 (显式 field, P1-2 Codex: 避免 lens/sidekick 字段混进同一对象)。
 * 借鉴 vocab unionReviewHistory 经验:
 *   - 一端 date 非法/缺失: 取合法端 (P1-1 防丢有效扣减; 不能因对端坏数据丢自己的扣减)
 *   - 两端合法且不同日: 取较新 date 全量 (YYYY-MM-DD 字典序即时间序)
 *   - 同日 (或两端都非法无法判新旧): union field 数组去重 (幂等不丢)
 *
 * @param {object|null} local
 * @param {object|null} remote
 * @param {string} field  'usedLensIds' | 'eventIds'
 * @returns {object|null}
 */
function mergeUsage(local, remote, field) {
  if (!local) return remote || null;
  if (!remote) return local;
  var lValid = isValidQuotaDate(local.date);
  var rValid = isValidQuotaDate(remote.date);
  // P1-1: 一端 date 非法 → 取合法端, 绝不因对端坏 date 丢掉自己有效的扣减
  if (lValid && !rValid) return local;
  if (rValid && !lValid) return remote;
  // 两端合法但不同日 → 较新 date 全量胜出
  if (lValid && rValid && local.date !== remote.date) {
    return (local.date > remote.date) ? local : remote;
  }
  // 同日, 或两端都非法 (无法判新旧 → 保守 union 不丢): union field 数组去重
  var seen = {};
  var union = [];
...
/**
 * 合并 lens usage ({date, usedLensIds})。sync 双端 / 多 tab 用。
 */
export function mergeLensUsage(local, remote) {
  return mergeUsage(local, remote, 'usedLensIds');
}

/**
 * 合并 sidekick usage ({date, eventIds})。eventIds union 是防 sync 双 +1 丢扣的核心。
 */
export function mergeSidekickUsage(local, remote) {
  return mergeUsage(local, remote, 'eventIds');
}
