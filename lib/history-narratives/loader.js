...
// ── 主 API：loadNarrative(topicId, lang?) ──
// lang: 'cn'（默认，加载 {topicId}.md）/ 'en'（加载 {topicId}.en.md）
// 'en' 文件不存在时 fallback 到中文版（向后兼容旧 Topic 没翻译时）
export var loadNarrative = function(topicId, lang) {
  if (!topicId) throw new Error('topicId required');
  lang = lang || 'cn';
  var fileName = (lang === 'en') ? (topicId + '.en.md') : (topicId + '.md');
  var filePath = path.join(NARRATIVE_DIR, fileName);
  if (!fs.existsSync(filePath)) {
    if (lang === 'en') {
      // EN 版还没翻译——fallback 到 CN 版
      var cnPath = path.join(NARRATIVE_DIR, topicId + '.md');
      if (fs.existsSync(cnPath)) {
        var cnRaw = fs.readFileSync(cnPath, 'utf-8');
        var parsed = parseRawNarrative(cnRaw);
        parsed.metadata.lang = 'cn';
        parsed.metadata.langFallback = true;  // 调用方可以提示用户"英文版未翻译，显示中文"
        return parsed;
      }
    }
