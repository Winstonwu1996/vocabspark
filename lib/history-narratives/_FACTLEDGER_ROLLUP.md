...
## 🅰 必修：硬史实错误（✋ 共 9 处）— ✅ 已全部修复（2026-05-23，commit 见下）

> 9 处硬错已定点修复（CN+EN+deliverGoal+kernel 一致），node --check + 37 storyboard 加载 +
> 各 lens 节点数/输入门结构全部验证通过；残留错误值 grep = 0。下表保留为审计记录。
