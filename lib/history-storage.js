...
import { mergeLensUsage, mergeSidekickUsage } from './daily-quota';
...
export var saveAll = function(data) {
  ...
  try {
    ...
    // Step 3 (Codex round6 P2-j): 每日配额字段是 union-monotonic, 但本 saveAll 整体替换 historyData
    // (浅合并)。其它 writer (saveInProgress / saveSidekickLog / saveLearningReceipt…) 读旧 blob 改自己
    // 字段再整体写回, 会把并发 (多 tab) 刚记的配额扣减覆盖丢。→ 凡写 historyData, 都把配额两字段与
    // 磁盘最新值 union-merge (幂等不丢扣), 让任何 writer 都不可能 clobber 掉配额。
    if (data && data.historyData && existing && existing.historyData) {
      var inHd = merged.historyData; // = data.historyData (浅合并后)
      var exHd = existing.historyData;
      if (inHd.dailyLensUsage || exHd.dailyLensUsage) {
        inHd.dailyLensUsage = mergeLensUsage(inHd.dailyLensUsage || null, exHd.dailyLensUsage || null);
      }
      if (inHd.dailySidekickUsage || exHd.dailySidekickUsage) {
        inHd.dailySidekickUsage = mergeSidekickUsage(inHd.dailySidekickUsage || null, exHd.dailySidekickUsage || null);
      }
    }
...
// ─── Step 10: 「自动推荐」开关 ──────────────────────────────────────
// 完成一门课时是否把错词自动推进 Vocab 桥队列。默认 on（与历史行为一致）。
// 纯 localStorage 设置，不含任何会员代码 —— 可在任意 bundle 安全 import。
// Codex round7 P2: 必须存在 historyData 里 —— mergeProgress 用白名单重建 blob, 顶层未知字段
// 会在 409/登录 sync 合并时被丢, 导致关掉的开关下次加载默认回 true 静默重新开启。
// historyData 走 mergeHistoryData 的 passthrough (newer 赢), 偏好得以跨设备/合并保留。
export var getHistoryAutoRecommend = function() {
  var all = loadAll();
  var hd = all && all.historyData;
  if (!hd || hd.autoRecommend === undefined || hd.autoRecommend === null) return true;
  return hd.autoRecommend !== false;
};

export var setHistoryAutoRecommend = function(enabled) {
  var all = loadAll() || {};
  var hd = (all.historyData && typeof all.historyData === "object") ? all.historyData : {};
  // 读现有 historyData 整体回写 (saveAll 浅合并会整体替换 historyData) —— 保住其它进度字段;
  // saveAll 内部还会 union-merge 配额两字段, 不丢并发扣减。
  saveAll({ historyData: Object.assign({}, hd, { autoRecommend: !!enabled }) });
};
...
// 5-4: 默认改 high（英文偏多）—— lens 内容是 prewritten 双语，high = EN, 其他 = CN
export var loadEnglishLevel = function() {
  var all = loadAll();
  return (all && all.historyData && all.historyData.englishLevel) || "high";
};
...
// ─── Learning Receipt（学习证据）──────────────────────────────────
// 见 docs/HISTORY_LEARNING_RECEIPT_DESIGN.md。每个 lens 走完最后一节、进 mastery 前，
// 学生留 4 件证据（事实锚点/因果/视角限制/英文表达）。纯附加字段
// historyData.learningReceipts[topicId][lensId]，与 completedTopics/inProgress 同级互不干涉，
// 不读不写 turnIndex；老用户无此字段时读路径一律 || null 兜底（零迁移）。
// lensId 缺失（旧无 lens topic）→ "__default__" 兜底，避免不同 lens 互相覆盖。
export var saveLearningReceipt = function(topicId, lensId, payload) {
  if (!topicId) return;
  var all = loadAll() || {};
  var historyData = all.historyData || {};
  historyData.learningReceipts = historyData.learningReceipts || {};
  historyData.learningReceipts[topicId] = historyData.learningReceipts[topicId] || {};
  var key = lensId || "__default__";
  historyData.learningReceipts[topicId][key] = Object.assign(
    {
      schemaVersion: 1,
      topicId: topicId,
      lensId: key,
      submittedAt: new Date().toISOString(),
      source: "lens-complete",
    },
    payload || {}
  );
  saveAll({ historyData: historyData }); // read-merge-write，纯附加
};
