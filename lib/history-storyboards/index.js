...
// ─── 全量加载（**只给 scripts/ 下的内容审计脚本用**）──
// 返回 { topicId: module } —— 等价于改造前的 STORYBOARDS 导出。
// ⚠️ 客户端代码永远不要调这个：它会把全部 71 门课(7.4MB)拉下来，
// 正是本次瘦身要消灭的东西。页面侧一律用 preloadStoryboard(当前这门)。
export var loadAllStoryboards = async function() {
  var out = {};
  var ids = Object.keys(LOADERS);
  for (var i = 0; i < ids.length; i++) {
    out[ids[i]] = await preloadStoryboard(ids[i]);
  }
  return out;
};
