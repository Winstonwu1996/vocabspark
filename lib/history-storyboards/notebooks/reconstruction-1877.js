...
I'll come back and check this list.`,
    ...
    keyFigures: [
      ...
      },
      {
        ...
        roleCn: '逃奴出身、自学成才的废奴/民权领袖，比 Revels 激烈（跨主题锚 A4）',
  ...
  mainConcepts: [
    // ── 卡 1：三条重建修正案（13/14/15）── 故事覆盖型（N6 三 lens 全挂）
    {
      ...
      storyAnchor: {
        ...
        xiaoweiNote: {
          cn: `这三条三个故事里都反复出现，是这一整门课的骨架，老师说必背：
...
却睁眼闭眼——这正好引出"有权阻止错事却袖手旁观，算不算共犯"这一课的核心拷问（DBQ 1）。`,
...
跨主题锚（fs-n8）：和 Black Death 后欧洲地主立法把农民绑在土地上是同一个套路——
...
佃农那一遍的节奏其实和真实时间线吻合：他"老来"才遇上识字测验——
...
"old age" — that composite spans c.1840-1910, and old age lands right in the 1890s.`,
    ...
    // ── 卡 9：Plessy v. Ferguson（1896）── 独立 mini-lesson（故事未展开，1877→1896→1954 链关键节点）
    {
      ...
      storyAnchor: {
        covered: false,
        mentionedIn: [
          { lens: 'freedman-sharecropper-receiving-end', nodeId: 'fs-n9', context: 'Jim Crow 隔离制度背景，Plessy 是司法背书那一刀' },
        ],
      },
      standaloneText: {
        cn: `这张卡故事没专门展开，但它是 APUSH 和 AP Gov 的超高频考点——1877→1896→1954 链的中间那颗钉。
...
考点关键句：Plessy 不是「重建时期」的事，是重建结束后近 20 年才发生的——这正好说明 1877 的代价如何**一层层累积**，直到 20 世纪中叶才被清算。`,
        en: `This card isn't given its own scene in the stories, but it's a top-frequency APUSH and AP Gov exam point — the middle nail in the 1877→1896→1954 chain.
...
**The 1877→1896→1954 chain (AP must-know causation)**:
...
Key exam sentence: Plessy is not a "Reconstruction era" event — it happened nearly 20 years after Reconstruction ended. That is exactly the point: the cost of 1877 accumulated layer by layer, not settled until the mid-20th century.`,
      ...
      xiaoweiNote: {
        cn: `老师的一句话总结：Plessy 是"写进纸上的平等被最高法院亲手按死"——
1877 撤军开门，1896 Plessy 上锁，1954 Brown 才把锁砸开。
AP 爱考"Harlan 的异见里哪句话最后被用来推翻 Plessy"：**「宪法是色盲的」**。`,
...
进阶（跨主题对照）：把 Revels 和 Frederick Douglass 对照——一个温和、一个激烈，
