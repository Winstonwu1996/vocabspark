...
each civilization.`,
...

    {
      ...
      storyAnchor: {
        ...
        xiaoweiNote: {
          cn: `始皇帝那一遍第 6 节和李斯那一遍第 6 节都讲了，是全 Topic 的「跨视角锚点」。
...
骊山陵（从即位起修几十年，最多役使号称七十万人）。骊山陵里就是震惊世界的兵马俑——
...
the emperor sees a great work, the conscript sees who is carrying it on his back beneath.`,
...

    {
      ...
      standaloneText: {
        cn: `这张卡把故事里散在各处的「秦怎么垮、又留下什么」串成一条完整时间线——
CA HSS 和中国课纲都要求会讲秦的速亡，AP 还爱考「短命王朝为什么留下长命制度」。

先记一条「崩」的时间线：
- 公元前 210 年：始皇帝第五次出巡途中死于沙丘（求长生反吃含汞丹药，死在路上，享年约 49）。
  遗诏要长子扶苏回来继位，可掌印宦官赵高拉拢丞相李斯，把遗诏改了——
  赐死扶苏，立易控制的幼子胡亥当二世皇帝。这叫「沙丘之变」。
- 公元前 209 年：一队戍卒因大雨误了报到期限——按秦法，误期当斩。
  横竖是死，陈胜、吴广索性起义，喊出「天下苦秦久矣」「王侯将相，宁有种乎」。星火燎原。
- 公元前 208 年：李斯被赵高构陷，腰斩于咸阳，株连全族。
- 公元前 206 年：秦亡。从始皇帝称帝（前 221）算起，连十五年都没到——妄想传万世，结果二世而亡。

再记一条「不死」的线（这才是 AP 高分要的反转）：
秦虽速亡，可它立的「制度」没跟着死。后来的汉朝，一边骂秦残暴，一边几乎原封不动地
继承了郡县制、统一的文字、度量衡——这叫「汉承秦制」。此后两千年，王朝一个接一个换，
可这套骨架基本没变。

考点关键句（必背反差）：「人没了，朝代没了，可秦造的那套制度，活了两千年。」
这正是 AP 的「continuity & change（延续与变化）」核心——
朝代是变化的，制度是延续的；一个短命的暴政，留下了长命的制度。

🔗 故事连接：始皇帝那一遍第 8 节（沙丘死、遗诏被改）、第 9 节（起义、秦亡、制度被汉继承）；
李斯那一遍第 8 节（他和赵高一起改遗诏的细节）；戍卒那一遍第 9 节
（「误期者斩」的严法既逼反戍卒、又点燃灭秦烈火的反讽）。`,
...
🔗 Story link: the First Emperor's Node 8 (death at Shaqiu, the altered edict) and Node 9 (revolt, fall, the
system inherited by the Han); Li Si's Node 8 (the details of how he and Zhao Gao altered the edict); the
conscript's Node 9 (the irony that the harsh "behead the latecomer" law both drove the conscripts to revolt
and lit the fire that destroyed Qin).`,
      },
      xiaoweiNote: {
