...

    {
      ...
      storyAnchor: {
        ...
        mentionedIn: [
          ...
          { lens: 'conscript-peasant-receiving-end', nodeId: 'cp-n11', context: '戍卒收尾追问「盛世是谁的盛世、谁在底下扛」——本卡片用同代罗马的扩张做的「谁买单」对照，问的正是同一个问题' },
        ],
      },
      standaloneText: {
...
**察举制（recommendation system）** 的运作方式：
- 地方官员（刺史 / 郡守）定期向朝廷**推荐**当地「孝廉」（孝顺且廉洁的人才）。
- 被举荐者经考核进入官僚体系；这把选官的权力部分集中在皇帝手里，打破了旧贵族世袭垄断官位的格局。
- 与设太学（国家培养儒生）配合，构成「读儒书 → 被地方举荐 → 进入官僚」的人才管道。

**司马迁（Sima Qian）的 sourcing 视角**（AP DBQ sourcing 训练）：
司马迁是当世史官，他因为替战败的李陵辩护得罪汉武帝，受**宫刑**。他写《史记》时对汉武帝多处持批评态度——同样一套「独尊儒术 + 察举制」，在庙堂看是「统一思想、选贤举能」，在司马迁笔下是「皇权借儒术压制异见」。读《史记》时要问：**作者是谁、他与皇帝的关系是什么**——这正是 AP sourcing 的核心。

考点关键句：察举制 = 地方举荐孝廉 + 中央考核 = 官僚选拔从世袭转向（部分）能力/儒学标准；配合设太学，是独尊儒术落地的制度底座。`,
        en: `This is another key mechanism by which Emperor Wu consolidated central authority — high-frequency in AP World "state-building" questions, but many students memorize only the Confucian doctrine and miss the accompanying official-selection system.

**The recommendation system (察举 jiucha)** in practice:
- Local officials (inspectors / commandery governors) periodically **recommended** talented locals recognized as "filially pious and incorruptible" (xiaolian) to the court.
- Recommended candidates entered the bureaucracy after review; this shifted some power over official selection to the emperor, breaking the old aristocracy's hereditary grip on office.
- Combined with the Imperial Academy (state training of Confucian scholars), it formed the pipeline: "read the Confucian classics → be recommended locally → enter the bureaucracy."

**Sima Qian's sourcing perspective** (AP DBQ sourcing practice):
Sima Qian was the court historian of this era. He offended Emperor Wu by defending the defeated general Li Ling and was sentenced to **castration**. When he wrote the Records of the Grand Historian (Shiji), he was critical of Emperor Wu in multiple places — the same system of "Confucianism alone + recommendation," seen from the court, is "unifying thought and selecting talent"; seen through Sima Qian's brush, it is "imperial power using Confucianism to suppress dissent." When reading the Shiji, ask: **who is the author, and what is his relationship to the emperor?** That is the core of AP sourcing.

Key exam sentence: the recommendation system = local recommendation of filially pious and incorruptible candidates + central review = official selection shifting (partly) from hereditary to ability/Confucian-learning criteria; combined with the Imperial Academy, it is the institutional base on which Confucianism alone landed.`,
      },
      xiaoweiNote: {
        cn: `老师说察举制和设太学要一起记：太学培养，察举推荐——两个齿轮咬合，才是「读书做官」这条路的完整机制。
司马迁那条 sourcing 提醒：他本人被皇帝处罚过——读他写的汉武帝，要知道这个作者的立场和处境。
...
The Sima Qian sourcing reminder: he himself was punished by the emperor — when reading what he wrote about Emperor Wu, know the author's position and circumstances.
Mnemonic: jiucha = local recommend filially pious → central review → enter office; Shiji sourcing = who is the author + what is his relationship to the emperor.`,
      },
    },
...
**背景**：汉初实行郡县制与分封制并行——皇帝直辖一部分土地，同时分封刘姓宗族为诸侯王。诸侯王领地大、军队强，对中央权威构成威胁。景帝时「七国之乱」（公元前 154 年）平定后，问题仍未根除。

**推恩令（Decree of Grace，公元前 127 年）** 的机制：
- 汉武帝下诏：诸侯王的封地，死后不得只传嫡长子，必须**分给所有儿子**（以示皇恩浩荡）。
- 结果：每传一代，诸侯国就被切割得更小；几代之后，诸侯王的领地缩成郡县规模，再也无法对抗中央。
- 妙处：诸侯王自己也说不出什么——这是皇帝的「恩典」，让所有儿子都有份。实际上是用「分」的方式瓦解了「封」的威胁。
...
**Background**: early Han ran a hybrid of commandery-county and feudal enfeoffment — the emperor directly governed some territory while granting Liu-clan relatives as feudal kings. These kings held large territories and strong armies, threatening central authority. After the "Revolt of the Seven States" (154 BCE) under Emperor Jing was suppressed, the problem remained.

**The Decree of Grace (推恩令 Tuien Ling, 127 BCE)** mechanism:
- Emperor Wu decreed: when a feudal king died, his domain could not pass only to the eldest son — it must be **divided among all sons** (presented as the emperor's generous favor).
...
- Elegance: the feudal kings could not object — this was the emperor's "grace," giving every son a share. In practice it used "division" to dissolve the threat of "enfeoffment."

Key exam sentence: the Decree of Grace = using "share the fief-grace among all heirs" to make kingdoms shrink generation by generation until dissolved — a political engineering of centralization, not military suppression. AP often tests "how Han consolidated unity more sustainably than Qin": the Decree of Grace is one answer.`,
      },
      xiaoweiNote: {
...
Mnemonic: Decree of Grace (127 BCE) = divide fief among all sons → kingdom shrinks each generation → centralization achieved without warfare.`,
...

    {
      ...
      storyAnchor: {
        ...
        nodeIds: ['zq-n9'],
        xiaoweiNote: {
          cn: `第 9 节提到路通罗马、良马东传，这张卡把「考试要记的货物单」补齐。

**向西（中国 → 地中海）**：丝绸（得名于此）、瓷器、漆器、铁器。
**向东（中亚/西亚 → 中国）**：
...
考点关键句：丝路三大货物 = 丝绸（西行）+ 汗血马（东行）+ 玻璃（东行）；佛教东传是最重要的文化结果。`,
          en: `Zhang Qian's lens Node 9 mentions the road reaching Rome and fine horses coming east, but doesn't give the full exam-ready list of goods. This card fills that gap.

AP World and CA HSS both test "what the Silk Road exchanged" — not just "silk" but **two-directional** specific goods and culture:

**Westward exports (China to Central / West Asia / Mediterranean)**
- **Silk**: the most important export, lightweight, high value, coveted by Roman elites; the road is named for it.
- Porcelain, lacquerware, ironware, tea (later period).

**Eastward imports (Central / West Asia into China)**
- **War horses / Ferghana horses (Heavenly horses, Dayuan horses)**: the resource Emperor Wu needed most for fighting the Xiongnu was cavalry horses; Zhang Qian brought back word that Dayuan (the Fergana Valley) had these prized horses, and Emperor Wu sent an army specifically for them.
- **Glass**: China could not mass-produce glass at this time; Western glass objects traveled the Silk Road into China and were treated as precious luxury goods.
- Grapes, alfalfa (quality fodder — a critical military resource), spices, cotton.

**Transmission of culture and religion** (super-high-frequency AP DBQ):
- **Buddhism** traveled the Silk Road from South Asia through Central Asia into China (spreading more widely after the Eastern Han).
- Artistic styles (Gandhara art), music, dyeing techniques flowed in both directions.

Clearing the trap: ❌ "the Silk Road was mainly Chinese exports" → ✅ it was **two-directional exchange** — goods, species, religions, and techniques all flowed both ways.
