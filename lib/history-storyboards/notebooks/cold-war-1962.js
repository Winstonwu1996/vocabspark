...
//        · 故事覆盖的：storyAnchor.covered = true + xiaoweiNote（lens+nodeId）
//        · 故事没覆盖的：storyAnchor.covered = false + standaloneText (mini-lesson 300-500 字)
//   3. thinkingQuestions（出口批判性思考）— 3 道 DBQ 入门题
//
// 与 storyboard 的关系（3 lens / 12 nodes each）：
//   - jfk (perpetrator-actor)              → 古巴危机决策 / 猪湾 / 越南 / 暗杀 / MAD / 1991
//   - vasili-arkhipov (lonely-mediator)    → B-59 那一票 / 核鱼雷 / 体制内抗命
//   - proxy-war-peasant-receiving-end      → 代理战争平民 / 棋盘上一格颜色 / 冷战的人命账
//   - notebook 补完课纲缺口：起源(雅尔塔/铁幕) / 遏制(杜鲁门主义+马歇尔计划) /
//     朝鲜战争 / 去殖民与冷战交织 / 不结盟运动
//
// 课纲对齐：
...
Let me write these down. After I read the three stories (Kennedy, the submarine
second-in-command, and a peasant caught in a proxy war), I'll come back and check
...
things. Your job isn't to pick a side — it's to understand both."`,
    ...
    keyFigures: [
      ...
      },
      {
        ...
        roleCn: '苏联潜艇分队参谋长（brigade chief of staff），随舰乘 B-59，1962 年三军官里唯一拒绝发射核鱼雷的人',
  ...
  mainConcepts: [
    // ── 卡 1：起源（雅尔塔 / 铁幕 / 意识形态）— 故事部分覆盖 ──
    {
      ...
      storyAnchor: {
        ...
        nodeIds: ['jfk-n2'],
        xiaoweiNote: {
          cn: `这个我在视角一第 2 节读到过——肯尼迪自己解释"我们为什么走到这一步"。
我把故事没细讲、但考试要补的几块拼上：
...
+ **铁幕**（一个比喻定了基调）。`,
          en: `I read this in Lens 1 Node 2 — Kennedy himself explains "why we came to this point."
...
Story link: Lens 1 Node 2 mentions the Truman Doctrine in one line, but the full
...
don't mix them up.`,
    ...
    // ── 卡 3：古巴导弹危机 1962 — 故事覆盖（双 lens）──
    {
      ...
      storyAnchor: {
        ...
        xiaoweiNote: {
          cn: `这个三个故事都讲到了，是整门课的核心。我把考试要的骨架拎出来：

**时间**：1962 年 10 月 16 日（侦察照片确认苏联在古巴架核导弹）→ 10 月 28 日
（赫鲁晓夫宣布撤），一共 **13 天**。古巴离美国海岸只有约 150 公里。

**肯尼迪的选择（视角一第 4、5 节）**：空袭、入侵、封锁、什么都不做——四条都通向核战。
他选了**封锁**，故意叫"隔离"（quarantine），因为"封锁"（blockade）在国际法上等于宣战。
他还私下答应撤掉美国在土耳其、对着苏联的导弹，给对方一个不丢脸的台阶。

**10 月 27 日"黑色星期六"（视角一第 6 节 / 视角二第 4-6 节）**：一架美国侦察机
在古巴被击落、飞行员死了；同一天，加勒比海面下苏联潜艇 B-59 里，三个军官在吵
要不要发射核鱼雷——分队参谋长 Arkhipov 投了唯一的反对票。

考点关键句：世界没毁灭，**不是单一英雄赢的**——是肯尼迪的克制 + 赫鲁晓夫的退让
+ 私下交易（撤土耳其导弹）+ Arkhipov 那一票 + 一点侥幸，多方因素加起来。
"古巴危机 = JFK 一个人冷静博弈赢的"是常见误解，别这么写。`,
          en: `All three stories cover this — it's the heart of the topic. Let me pull out
...
**Kennedy's options (Lens 1 Nodes 4-5)**: air strike, invasion, blockade, do nothing —
...
**"Black Saturday," 27 October (Lens 1 Node 6 / Lens 2 Nodes 4-6)**: an American spy
plane was shot down over Cuba and the pilot killed; that same day, beneath the Caribbean
inside Soviet submarine B-59, three officers argued over firing a nuclear torpedo —
second-in-command Arkhipov cast the only "no."
...
但代价是把暴力**挤压到第三世界**（看视角三的农民），还让全人类长期活在核阴影下。
"MAD 很疯狂所以没用"是误解——别这么简单地写。

跨课锚：原子弹（二战）→ MAD（冷战）是直接续接。`,
          en: `Lens 1 Node 6 explains this best. Here are the definition and exam points:
...
squeezing the violence **into the Third World** (see the peasant in Lens 3) and keeping
...
Cross-topic anchor: the atomic bomb (WWII) → MAD (Cold War) connect directly.`,
    ...
    // ── 卡 5：代理战争（朝鲜 / 越南）— 部分覆盖 ──
    {
      ...
      storyAnchor: {
        ...
        xiaoweiNote: {
          cn: `视角一第 8 节讲了代理战争和越南，视角三整条线就是代理战争里的农民。
故事没单独讲**朝鲜战争**，我把它补上：
...
**越南**：故事里讲了——北越（共产，苏中支持）vs 南越（亲美）。肯尼迪信
**多米诺理论**（一国倒向共产主义会带倒一片），一步步派"军事顾问"。
事实对账（重要）：1962 年底约 1.1 万美军顾问，到 1963 年底约 1.6 万，
肯尼迪 1963 年 11 月遇刺时约 **一万五千**名。他死后越战越打越大，
最终几十万美军陷入、几百万越南人死亡。

考点关键句：代理战争里，**真正付代价的是第三世界平民**（视角三的农民）——
死的平民远多于两个超级大国自己的士兵。`,
          en: `Lens 1 Node 8 covers proxy war and Vietnam, and all of Lens 3 is a peasant
caught in a proxy war. The story doesn't separately cover the **Korean War**, so let me
add it:
...
**Vietnam**: the story covers this — North Vietnam (communist, backed by the USSR and
China) vs South Vietnam (pro-American). Kennedy believed the **domino theory** (one
country falling to communism topples a row) and step by step sent "military advisers."
...
(the peasant in Lens 3) — far more civilians died than soldiers of the two superpowers.`,
        },
      },
    },
    ...
    // ── 卡 6：去殖民与冷战交织 / 不结盟运动 — 独立 mini-lesson ──
    {
      ...
      standaloneText: {
        cn: `这块故事只在视角三第 9 节点了一下"殖民→冷战是同一条线"，
...
1955 年印尼万隆会议（Bandung Conference）是起点，1961 年正式成立。
代表人物有印度的尼赫鲁、埃及的纳赛尔、南斯拉夫的铁托等。
...
它和美国课堂"自由世界 vs 极权"的两极框架，切分世界的方式根本不同——两套都要懂。`,
        en: `The story only touches "empire → Cold War is one line" in Lens 3 Node 9, but
the **Non-Aligned Movement** isn't covered on its own — and it's a required AP Unit 8
...
Cold War. So many new nations' internal struggles (who rules, whether to "divide the
land") got forced into the frame of "capitalism or communism" and turned into proxy wars.
(This is exactly the peasant's experience in Lens 3.)

**Non-Aligned Movement**: an effort by newly independent nations to **refuse to pick a
side**. The 1955 Bandung Conference in Indonesia was its start; it was formally founded
in 1961. Leading figures included Nehru of India, Nasser of Egypt, and Tito of Yugoslavia.
...
**Bandung 1955 (the start), formally founded 1961, three faces: Nehru / Nasser / Tito.**`,
    ...
    // ── 卡 7：冷战结束 1991 — 故事覆盖 ──
    {
      ...
      storyAnchor: {
        ...
        xiaoweiNote: {
          cn: `视角一第 10 节肯尼迪替我们把故事讲到头，视角三第 10、11 节农民从底层看结局。
考点骨架：
...
- 从白宫/全球看（视角一）：1991 没有蘑菇云，人类躲过核毁灭，值得庆幸。
- 从代理战争的村子看（视角三）：大国一走，留下的是打烂的国家、埋在地里的地雷、
  回不去的难民、几代人的仇恨——对他们，仗没那么干脆地"结束"。

考点关键句：冷战的"结束"对超级大国和对第三世界，是**两件不一样的事**。`,
          en: `Lens 1 Node 10 has Kennedy carry the story to its close for us, and Lens 3
Nodes 10-11 show the ending from the bottom, through the peasant. Exam skeleton:
...
- From the White House / globally (Lens 1): 1991 brought no mushroom cloud, humankind
  dodged nuclear ruin — something to be thankful for.
- From a proxy-war village (Lens 3): when the great powers left, what remained was
  wrecked countries, buried landmines, refugees who couldn't go home, and generations of
...
and for the Third World.`,
    ...
    // ── 卡 8（新增）：柏林危机 — 封锁与空运（1948-49）+ 柏林墙（1961）— 独立 mini-lesson ──
    {
      id: 'berlin-crisis',
...
Rule 0：封锁和建墙在苏联叙事里是防御（阻止人才资本外流），不是单纯挑衅。两种读法都要会写。`,
        en: `Berlin is the most concrete battlefield of the early Cold War's two-camp standoff —
not trenches, but a city. Both AP World and APUSH test these two episodes; the story
doesn't cover them, so this card fills in the gap.

**Background**: after WWII, Germany was divided among the US, Britain, France, and the USSR.
Berlin, though deep inside the Soviet zone, was similarly split four ways. West Berlin
became a capitalist-camp "island" inside Soviet-controlled territory.

**The Berlin Blockade and Airlift (1948-49)**
In June 1948 the USSR blocked all land routes to West Berlin, trying to force the West to
give up the city — cutting off food, fuel, and supplies.
...
(many of them educated professionals). In the early hours of 13 August 1961, East Germany —
with Soviet backing — strung barbed wire and poured concrete, sealing off West Berlin.
The wall cut straight through the city; families, neighbors, streets were split overnight.

**Key exam sentences**:
- Berlin Blockade = the **first physical test** of the containment strategy: the USSR probed the
  West's limits with force; America held the line with the airlift.
...
and "was the wall 1948 or 1961" (only 1961).`,
    ...
    // ── 卡 9（新增）：麦卡锡主义（McCarthyism）— 独立 mini-lesson ──
    {
      id: 'mccarthyism',
...
- Rule 0：「大多数指控无据」和「苏联确实有间谍网络（Hiss、Rosenbergs）」同时为真。
  高分答案不简化成「全是迫害」或「全有道理」。`,
        en: `The three story lenses focus on the Cuban Crisis and proxy wars — the "external"
Cold War. But APUSH equally tests the Cold War's "domestic" dimension: a political purge
inside America fueled by anticommunist fear.

**The Second Red Scare (1947-1954)**
After WWII, the USSR tested an atomic bomb (1949), China "fell" (1949), and the Korean
War began (1950), convincing many Americans that communism was infiltrating the country.
Congress launched loyalty investigations; FBI director J. Edgar Hoover hunted suspected
...
was often enough to destroy a career.
Hollywood's "blacklist" left many screenwriters and directors unable to work; civil servants
were fired for insufficient demonstrations of loyalty.
...
- It demonstrates that external threat (Cold War fear) can erode civil liberties inside a
  democracy.
- Rule 0 key: McCarthy collapsed, but real Soviet spy cases (Alger Hiss, the Rosenbergs)
  coexisted with his era — "most McCarthy accusations lacked evidence" and "the USSR did run
  spy networks" are both simultaneously true. A high-scoring answer does not collapse this
  into "all persecution" or "all justified."`,
      },
      xiaoweiNote: {
...
My mnemonic: **1950 McCarthy takes the stage with a list → 1954 televised hearings bring him down →
Senate censure the same year.**
Multiple choice loves "who ended McCarthyism" — answer: the televised hearings (the whole
country watched him in action).`,
      },
    },

    // ── 卡 10：冷战的人命账 / 双框架中立 — 故事覆盖（综合卡）──
    {
...
- 美国：猪湾入侵、暗杀外国领导人计划、支持反共独裁政权、越南升级（视角一第 8、9 节）
- 苏联：1956 年开坦克镇压匈牙利、扶持自己阵营的独裁者、把核鱼雷开到别人家门口
  （视角二第 11 节 Arkhipov 自己承认"我的军队"也是疯狂体制一部分）
视角一第 9 节肯尼迪直接说："两个都自认正义的大国，都在自己阵营里做着同样的事。"
...
写 essay 模板：
1. 美苏各列至少 2 件"自认正义却踩着别人"的事（对称，别只批一边）。
2. 用视角三农民给"代价"上人脸——别只写数字。
3. 结论：用 Arkhipov（救世界的是苏联军官）+ JFK（理想总统也搞猪湾/越南）
   双向打破"哪一边是无条件正义"。
4. 不下"谁对谁错"的死结论——这正是高分答案的标志。`,
...
- America: the Bay of Pigs invasion, plans to assassinate foreign leaders, backing
  anti-communist dictatorships, escalating Vietnam (Lens 1 Nodes 8-9)
- The USSR: rolling tanks into Hungary in 1956, propping up dictators in its own camp,
  sailing a nuclear torpedo to someone else's doorstep (Lens 2 Node 11, where Arkhipov
  himself admits "my own military" was part of the mad system too)
In Lens 1 Node 9, Kennedy says outright: "Both powers, each certain of its own justice,
did the same things inside their own camps."

**The human ledger (Lens 3 Nodes 6-8)**:
...
Essay template:
1. List at least 2 "sure of its justice yet trampling others" acts for each of the US and
   USSR (symmetrical — don't criticize only one side).
2. Use the Lens 3 peasant to put a human face on the "cost" — don't write only numbers.
3. Conclusion: use Arkhipov (the world-saver was a Soviet officer) + JFK (the idealist
   president also did the Bay of Pigs / Vietnam) to break "which side was unconditionally
   just" in both directions.
4. Don't draw a hard "who was right" conclusion — that restraint is the mark of a
   top-scoring answer.`,
        },
      },
    },
...
a possible future nuclear crisis?`,
      hintCn: `提示：可以从两个角度想——(1) 如果是"智慧赢的"，那我们可以相信"只要选好领导人，
核危机就能化解"；(2) 如果是"侥幸活的"，那真正的教训是"武器堆到这个地步本身就太危险"，
该想办法减少核武，而不是指望每次都遇到冷静的人。

进阶思考：把人类的安全押在"关键时刻总有聪明/冷静的人"上，靠谱吗？
还是说，更该靠"制度设计"（比如热线、限核条约）来降低出错的概率？`,
      hintEn: `Hint: think from two angles — (1) if it was "won by wisdom," we can trust that
"as long as we pick good leaders, nuclear crises can be resolved"; (2) if it was "survived
by luck," the real lesson is "piling weapons this high is itself too dangerous," and we
should reduce them rather than count on always meeting a calm person.
...
（视角三那个农民的村子，就是为一场他根本不认识的大国争斗被烧掉的。）
...
(The peasant's village in Lens 3 was burned for a great-power struggle he never even
recognized.)
...
是同一种"超级大国领袖"的两面（视角一第 11 节肯尼迪自己说"同一双手"）。
...
of the same "superpower leader" (Kennedy himself says "the same two hands" in Lens 1 Node 11).
