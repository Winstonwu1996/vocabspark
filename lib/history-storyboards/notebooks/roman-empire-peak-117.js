...
slaves, never counted in.`,
...

    {
      ...
      termCn: 'CCOT：共和国到帝国，变了什么、延续了什么',
      termEn: 'CCOT: Republic to Empire — What Changed, What Continued',
      standardRef: ['AP World Unit 1 — CCOT core skill', 'CA HSS 6.7', 'AP DBQ rubric'],
      examFrequency: 'highest',
      storyAnchor: {
        covered: false,
        mentionedIn: [
          { lens: 'trajan', nodeId: 'tr-n2', context: 'Trajan 自述接手一台已经造好的帝国机器，暗示延续性' },
          { lens: 'provincial-elite', nodeId: 'pe-n2', context: '行省精英谈公民权扩展，涉及变化' },
        ],
      },
      standaloneText: {
        cn: `AP World 最爱考的写作框架之一是 CCOT（Continuity and Change Over Time，
延续与变化）。本课从共和国（Republic）到帝国（Empire）的转变，是练习 CCOT 的标准案例。

**变了什么（Change）**

1. 最高权力的来源：共和国由元老院（Senate）和两名执政官（consuls）轮流掌权，
   任期一年互相制衡；帝国由一个人（皇帝）实际说了算，终身制，
   权力来自军团效忠和个人威望，而非法律授权。
2. 合法性叙事：共和国以"公民共同体自治"为合法性基础；
   帝国用"最高统帅（princeps）为公民服务"来包装，奥古斯都始终拒绝"国王（rex）"称号，
   用"第一公民（princeps civium）"掩盖实质上的独裁。
3. 军事指挥权：共和国执政官在任期内指挥军队，任期结束权力终止；
   帝国皇帝是永久的最高统帅，军团直接向他宣誓效忠。

**延续了什么（Continuity）**

1. 元老院依然存在，执政官头衔依然选出，但都成了形式——真正的权力已转移。
   这是刻意保留的"外壳"，帮助皇帝维持共和国的合法性形象。
2. 罗马法（Roman law）体系延续，仍是帝国治理的骨干；
   公民权概念延续（只是后来逐步扩展）。
3. 行省制度、军团结构、道路工程体系——这些共和晚期发展成熟的治理工具，
   帝国全部继承并扩大。
4. 社会阶层：元老阶层（senators）、骑士阶层（equestrians）、平民、奴隶的
   基本等级结构延续；奴隶制作为经济地基延续。
...
第四步，写分析：为什么延续这么多——因为奥古斯都懂得"换汤不换药"是最稳的权力策略，
彻底革命反而会触发元老阶层的全面反抗。

考点陷阱：不能说"共和国变成帝国所以一切都变了"，也不能说"元老院还在所以没有变"。
CCOT 的分数来自同时抓住两面，并能解释"为什么变""为什么不变"。`,
        en: `One of AP World's most tested essay frameworks is CCOT (Continuity and Change Over Time).
The shift from Republic to Empire is a standard CCOT case study.

**What Changed**

1. The source of supreme power: the Republic's power rotated among two consuls and the Senate,
   each checked for one-year terms; the Empire's power rested in one man for life, derived from
   army loyalty and personal authority, not legal mandate.
2. The legitimacy narrative: the Republic claimed "self-governance by the citizen community";
   the Empire wrapped power in "the first citizen (princeps) serving the people." Augustus
   refused the title "king (rex)" and used "first citizen (princeps civium)" to conceal what
   was in effect autocratic rule.
3. Military command: Republican consuls commanded armies during their term, then surrendered
   that power; the emperor was permanent supreme commander, with legions swearing loyalty
   directly to him.

**What Continued**

1. The Senate still met, consuls were still elected — but both became ceremonial. This was a
   deliberately preserved "shell" to let emperors claim the Republic's legitimacy.
2. The Roman law system continued as the backbone of governance; the concept of citizenship
   continued (though extended later).
3. The provincial system, the legionary structure, and the road engineering network — all
   these governing tools matured in the late Republic, inherited and enlarged by the Empire.
4. The basic social order: senatorial class, equestrian class, commoners, slaves — the core
   hierarchy continued; slavery as the economic foundation continued.

**CCOT Writing Template (four steps)**

Step 1, frame the span and context: Republic (509 BCE) to Empire (27 BCE, Augustus);
Step 2, name the changes: supreme power moved from shared to one-man, legitimacy narrative shifted;
Step 3, name the continuities: the Senate shell, Roman law, the province-legion-road system;
Step 4, analyze: why so much continuity? Because Augustus understood that "changing the substance
while keeping the form" was the most stable power strategy — outright revolution would have triggered
an all-out backlash from the senatorial class.

Exam trap: don't write "the Republic became the Empire so everything changed," and don't write
"the Senate still existed so nothing changed." CCOT points come from holding both sides at once
and explaining "why it changed" and "why it didn't change."`,
      },
      xiaoweiNote: {
        cn: `老师说 AP 的 CCOT 题最容易丢分的地方不是背不出事实，是只写变化忘了延续，
或者只写延续忘了变化。我的口诀：**"CCOT = 变了一半，留了一半，全都要说清楚为什么。"**

还有一个经典考题变种："奥古斯都是在结束共和制还是在延续共和制的外壳？"
标准答案是两者兼有——他实质上结束了共和制，但刻意保留共和形式以维持合法性。
这种"形变而实不变"的分析，就是 AP 要的 complexity（复杂性）。`,
        en: `Teacher said the most common way to lose CCOT points isn't forgetting facts —
it's writing only change and forgetting continuity, or vice versa. My mnemonic:
"CCOT = half changed, half stayed, explain both."

A classic question variant: "Was Augustus ending the Republic or continuing its shell?"
The standard answer is both: he effectively ended the Republic but deliberately kept its
forms to maintain legitimacy. That analysis of "form preserved, substance replaced"
is exactly the complexity AP rewards.`,
      },
    },
