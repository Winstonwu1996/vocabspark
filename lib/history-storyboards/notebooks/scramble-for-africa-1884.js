  ...
  mainConcepts: [
    {
      ...
      storyAnchor: {
        ...
        xiaoweiNote: {
          cn: `这个我读过！在视角一第 4 节，Leopold 雇 Stanley 跟不识字的酋长一个村一个村签"条约"，
...
see the next card + what you read in the villager's pass.)`,
...

    {
      ...
      storyAnchor: {
        ...
        xiaoweiNote: {
          cn: `这条线三个视角都讲到：Leopold 那边是账本上的数字，村民那边是地上的代价。
...
🔗 跨主题：这套话术不是 1876 才发明的。1492 年 Columbus 到美洲，殖民者就说去"教化"
原住民（见 age-of-exploration-1492 那段历史）。四百年后 Leopold 原样搬到非洲——
同一套"文明"修辞，用了快四百年。
...
- 1895 年甲午战争后，列强掀起「瓜分中国」狂潮，在地图上划「势力范围」——跟柏林会议上瓜分非洲是同一种动作。
- 租界、租借地、不平等条约：列强在中国攫取特权，与在非洲攫取土地同出一源。

两种极端都拒绝：
- 不能说「中国和非洲落后挨打，是因为没现代化」——这是把暴行归咎于受害者。
- 也不能说「西方天生贪婪，东方天生受害」——这抹掉了各方内部的揭发者与反抗者（义和团、辛亥革命；Williams 和 Casement 来自欧美）。

中性结论：新帝国主义是工业化、民族主义与种族等级论合力的产物；受害的一方既被侵略也在反抗，施暴的一方内部也有人揭发。历史的复杂性在各方都成立。`,
        en: `1870-1914 was the late Qing era. The same "new imperialism" logic was pressing down on China:

- After the 1895 First Sino-Japanese War, the powers launched a scramble to carve China into "spheres of influence" — the same move as drawing lines across Africa at the Berlin Conference.
...
Two extremes to reject:
- Don't write "China and Africa were beaten because they weren't modernized" — that blames the victim for the crime.
- Don't write "the West was inherently greedy, the East was purely victimized" — that erases the whistleblowers and resisters on all sides (Boxer Uprising, 1911 Revolution; Williams and Casement were Western).

Neutral framing: new imperialism was the product of industrialization, nationalism, and racial hierarchy theory combined; the victimized side was both invaded and resisting; even within the aggressor side, people spoke out. Historical complexity holds on every side.`,
      },
      xiaoweiNote: {
        cn: `这张卡对华裔学生特别有感——「瓜分非洲」和「瓜分中国」不是两件事，是同一套逻辑在两个大洲同时上演。你家里的历史课本可能把这段叫「帝国主义侵华」，美国课本叫「新帝国主义」，名字不同，说的是同一件事。`,
        en: `This card hits differently for Chinese-heritage students — "the Scramble for Africa" and the "carving up of China" aren't two separate things. They're the same logic playing out on two continents at the same time. Your family's history textbook might call this "imperialist aggression against China"; the American textbook calls it "new imperialism." Different name, same event.`,
      },
    },
  ],
