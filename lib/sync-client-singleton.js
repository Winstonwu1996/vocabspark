...
function getSyncClient(options, _factory) {
  ...
  var proxied = {
    ...
    // Codex P1: 让客户端在「每次同步操作开始」钉死当前活跃页回调 (防 in-flight 期间切页混用两页回调)。
    __getActiveCallbacks: function () { return _activeCallbacks; },
  };
...
// 全局登出兜底 (生产): 重锁共享实例的 push 闸门 + 清版本号。
// Codex P1: 单例 cloudReady=true 会跨登出残留; 若换账号先开 /history (在 /vocab 之前),
// history 的 pushHistorySync 会绕过首拉闸门, 用前一账号的就绪态把本地 blob 推到新账号 token 下。
// SIGNED_OUT 时调本函数 → cloudReady 复位 false, 下个用户必须自己拉云解锁才能推 (vocab 拉云会 setCloudReady)。
function resetCloudReadyOnSignOut() {
  if (!_instance) return;
  try { if (typeof _instance.setCloudReady === 'function') _instance.setCloudReady(false); } catch (e) {}
  try { if (typeof _instance.setSyncVersion === 'function') _instance.setSyncVersion(0); } catch (e) {}
}

// 测试 teardown (生产不调)。
function __resetSyncClientSingleton() { _instance = null; _activeCallbacks = null; _ownerLabel = null; }

export { getSyncClient, getActiveOwner, resetCloudReadyOnSignOut, __resetSyncClientSingleton };
