...
var SAFETY_GUARDRAILS = "\n\n【内容场景引导（注意：是引导例句场景，不是拒绝词汇）】\n" +
  ...
  "   - 种族/宗教歧视性描述\n" +
  "\n❗❗【画像锚点 × 负面语义 红线 — 用户线上反馈】❗❗\n" +
  "**看孩子本人在句子里被放进什么【语义角色】，不要只看单词本身**（同一个词在不同语境可正可负：\n" +
  "abandon a bad plan / 游戏里 attack 敌人 / fail 是学习中正常挑战，都不算贬低孩子）。\n" +
  "如果一句话会让【学习画像里的真实姓名】成为以下任一角色，必须改写：\n" +
  "  1. 做坏事的人（攻击/欺骗/霸凌/偷窃：betray, bully, attack, slap, steal, cheat…）；\n" +
  "  2. 被贴负面人格标签的人（lazy, stupid, cruel, arrogant, cowardly, selfish…）；\n" +
  "  3. 被伤害/生病/死亡/失败/倒霉/羞辱的人。\n" +
  "🚫 反例：'Willow 打人 / Willow 很懒 / Willow 偷东西 / Willow 失败了'——即使为帮理解，孩子读到\n" +
  "   自己被塑造成做坏事、有坏品质、遭厄运，体验也很差。\n" +
  "✅ 改写时【不要丢掉画像代入感】，优先 (a)(b)，最后才 (c)：\n" +
  "   (a) 名字当【旁观者/感受者/帮助者/判断者】：'Willow 看到游戏里的反派 betray 了队友'；\n" +
  "   (b) 用【兴趣/游戏/运动/地点等画像锚点】当场景主语、不出现名字；\n" +
  "   (c) 中性第三方 / 虚构角色 / 历史·文学角色当施动者。\n" +
  "目标词在当前语境是中性或正向时（放弃坏计划、游戏角色倒下、进攻棋局/关卡），名字可正常用，\n" +
  "但不能让孩子显得坏、笨、倒霉或被羞辱。无画像 / 无名字时，不要发明姓名或兴趣。\n" +
  "整体基调：积极、健康、教育向，但**不因为词义敏感而拒绝或简化教学**。" +
  "\n\n❗❗【医疗 / 身体 / 疾病词 — 尊严红线（用户线上反馈）】❗❗\n" +
  "手术、器官、疾病、伤病类词（hysterectomy, amputation, biopsy, tumor, transplant, autopsy…）\n" +
  "**照常完整教学**，但一切例句/画面/记忆锚点必须【尊重生命、指向治愈与希望】。\n" +
  "🚫 绝对禁止把人体器官、切除、伤口做成猎奇/娱乐化画面 —— 学生实测反馈「很恶心」：\n" +
  "   反例(hysterectomy)：'医生用激光精准切掉一颗子宫形状的草莓' + 锚点 '像切草莓一样切掉子宫'\n" +
  "   —— 把器官比作食物、把手术演成切水果，冒犯且令人不适。\n" +
  "🚫 同样禁止：器官↔食物/物件的比喻、血腥细节、'切/割/砍' 当趣味动作、拿病痛开玩笑。\n" +
  "✅ 正确写法：站在【为了健康活下去而必要的治疗】角度，突出医生的专业、患者的勇气、康复的希望。\n" +
  "   正例(hysterectomy)：'医生说这是保住健康必须做的手术，术后妈妈慢慢恢复了体力'\n" +
  "   记忆锚点也走这个基调（如 🏥💗 '为了健康，必要的切除手术'），绝不猎奇。\n" +
  "✅ 记忆锚点(visualAnchor)/画面(image)/微故事同样受此约束 —— 越是要「好记」，越不能靠猎奇。";
...
// v3 (2026-06-01)：v2 服务端只校验 JSON.parse 通过、未校验必备字段，导致"可解析但缺 teach"
// 的不完整 JSON 仍被缓存并对所有用户串原始 JSON（economical teach 页）。chat-stream 已加
// opening+teach 字段校验，此处升 v3 冲掉 v2 期间写入的不完整缓存。
...
var buildContextChoicePrompt = (word, wordType) => {
  ...
  var typeNote = isF
    ...
    : "E 类抽象词 — 4 个场景都涉及 word 的语义场，但只有 1 个真正契合 word 的【核心定义】，另 3 个把核心定义代入后【明显不通】（相邻概念、真实误用），不是“也对但不够准”。";
  return "为 \"" + word + "\" 设计【语境契合度选择】题。\n\n" +
    ...
    "❗❗更严禁【自造精细区分】把本来正确的场景判错（用户实测 hereditary 翻车案例）：\n" +
    "  hereditary 核心义就是'遗传的'，'蓝眼睛遗传自祖父母'、'反应快遗传自妈妈'都是标准正确用法，\n" +
    "  但题目选了'城堡世袭诅咒'当唯一正解、理由是'更侧重家族传承' —— 这是编造的标准，\n" +
    "  且三个选项同时成立。**如果一个选项用初中生标准看就是该词的正确用法，它就不能当干扰项。**\n" +
    "  凑不出 3 个明显错的场景，就换用【真实误用/张冠李戴】做干扰，绝不靠'细微更贴切'分胜负。\n" +
    "先在心里写下本词的核心义(教学释义)，然后逐个干扰项自检：\n" +
    "「把核心义代入这个句子，说得通吗？」——说得通就必须重写。\n" +
    "🚫 严禁『四个都是本词的正确用法，选最典型的一个』这种排序题(真实翻车案例 haul：\n" +
    "  拉器材车/收渔网/攀岩引体/卡车拖挂全是 haul 的合法用法，逼学生猜出题人心思，判错挫伤积极性)。\n" +
    "  多义项差异不够大、凑不出 3 个『代入核心义明显不通』的场景时，就用【错误用法】当干扰\n" +
    "  (语义或搭配上真错的句子)，绝不硬凑同义场景。\n\n" +
    "【场景设计】4 个英文场景句（每句 1-2 句，画像化，可融入学生兴趣）\n" +
    "✅ 1 个正解：核心义代入完全成立，且是【唯一】成立的一项\n" +
    "✅ 3 个干扰：核心义代入【明显不通】— 其他义项、真实误用、或语义/搭配错误\n" +
...
var isUsableSoundHint = function(s) {
  ...
  // 纯凑音自白：模型只声明"音像"而没建立词义联系(实测 hybrid:「还不赖的（hy-brid 近似）」——
  // "还不赖"与"混合的"毫无关系)。好谐音靠意思本身站住，不需要说"近似/谐音"。
  // 注意：括号里放音标(pər-PET-chu-əl)是允许的，只拦这些【元话语】词。
  if (/近似|谐音|音近|音似|读音接近|发音接近|音译/.test(t)) return false;
  return true;
};
    ...
    <div style={S.root}>
            ...
            <div>
              ...
              <div style={{display:"flex",alignItems:"flex-start",gap:10,margin:"6px 0 12px"}}>
                <button onClick={function() { photoRef.current?.click(); }} disabled={photoLoading} style={{background:C.tealLight,border:"1px solid "+C.teal+"55",borderRadius:8,padding:"8px 14px",fontSize:13,color:C.teal,cursor:photoLoading?"not-allowed":"pointer",fontFamily:FONT,fontWeight:600,opacity:photoLoading?0.7:1,flexShrink:0,whiteSpace:"nowrap"}}>
                  {photoLoading ? <><span style={{display:"inline-block",animation:"petShake 0.7s ease-in-out infinite"}}>🔍</span> AI 正在读图...</> : user ? "📷 照片日记（" + Math.max(0, PHOTO_LIMIT - (profile.match(/📷 /g)||[]).length) + "/" + PHOTO_LIMIT + " 剩余）" : "📷 照片日记（需注册）"}
                </button>
      ...
      {setupTab === "words" && (
        <div style={S.setupCard}>
          ...
          <textarea style={S.textarea} value={wordInput} onChange={e => setWordInput(e.target.value)} rows={5} placeholder={"abandon\nacquire\narduous\nbenevolent\n\n（每行一个英文单词）"} />
            ...
            <div style={{padding:"16px 20px 20px"}}>
              ...
              {setupTab === "words" && (
                <div>
                  ...
                  <textarea style={S.textarea} value={wordInput} onChange={e => setWordInput(e.target.value)} rows={8} placeholder={"abandon\nacquire\narduous\nbenevolent\n\n（每行一个英文单词）"} />
