...
import { hasNotebook, loadNotebook, preloadNotebook } from '../../lib/history-storyboards/notebooks/index.js';
  ...
  useEffect(function () {
    ...
    historySyncRef.current = getSyncClient({
      ...
      onSyncStatus: function (status, meta) {
        setHistorySyncState(status);
        if (meta && typeof meta.lastSyncAt === 'number' && meta.lastSyncAt > 0) setHistoryLastSyncAt(meta.lastSyncAt);
      },
  ...
  useEffect(function() {
    ...
    try {
      ...
      if (t && getTopic(t)) {
        ...
        var resolved = t;
        // notebook 同样按需（考点卡/mastery 门要用），和 storyboard 一起并行拉
        Promise.all([
          preloadTopicStoryboard(resolved),
          preloadNotebook(resolved).catch(function(e) {
            console.error("[history] notebook chunk 加载失败:", resolved, e && e.message);
          }),
        ])
          .catch(function(e) {
      ...
      <div className="h-page">
        ...
        {!embedded && user && historySyncState && !showUserCenter && (
          <div style={{
            ...
            fontFamily: FONT, boxShadow: "0 4px 16px rgba(0,0,0,0.2)",
          }}>
            {historySyncState === 'syncing' ? '↻ 同步中…'
              : historySyncState === 'error' ? '⚠ 同步未成功'
              : (function () {
                  var rel = '';
                  if (historyLastSyncAt) {
                    var s = Math.floor((Date.now() - historyLastSyncAt) / 1000);
                    rel = s < 60 ? '刚刚' : (s < 3600 ? Math.floor(s / 60) + ' 分钟前' : Math.floor(s / 3600) + ' 小时前');
                  }
                  return '✓ 已同步' + (rel ? ' · ' + rel : '');
                })()}
          </div>
